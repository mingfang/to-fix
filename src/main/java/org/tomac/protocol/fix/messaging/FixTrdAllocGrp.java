...
public class FixTrdAllocGrp extends FixGroup {
        			...
        			if ( tag == FixTags.NONESTED2PARTYIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !nestedParties2[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NONESTED2PARTYIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getAllocClearingFeeIndicator() { 		
		if ( hasAllocClearingFeeIndicator()) {		
			...
			FixUtils.getTagStringValue(buf, allocClearingFeeIndicator, 0, allocClearingFeeIndicator.length, err);
