        			...
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !rgstDtlsGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOREGISTDTLS_INT);
        						return;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !rgstDistInstGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NODISTRIBINSTS_INT);
        						return;
        					}
        					count++;
