...
public class FixTradeCaptureReport extends FixInMessage {
	...
	FixTrdInstrmtLegGrp[] trdInstrmtLegGrp;
	FixTrdRegTimestamps[] trdRegTimestamps;
	FixTrdCapRptSideGrp[] trdCapRptSideGrp;
	FixTrdRepIndicatorsGrp[] trdRepIndicatorsGrp;
	...
	public void setTZTransactTime(byte[] src) {		
		if (src == null ) return;
		if (hasTZTransactTime()) FixUtils.fillSpace(tZTransactTime);		
	...
	public void setRejectText(byte[] src) {		
		if (src == null ) return;
		if (hasRejectText()) FixUtils.fillSpace(rejectText);		
