...
public class FixInstrumentParties extends FixGroup {
        			...
        			if ( tag == FixTags.NOINSTRUMENTPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !instrumentPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOINSTRUMENTPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getInstrumentPartyID() { 		
		if ( hasInstrumentPartyID()) {		
			...
			FixUtils.getTagStringValue(buf, instrumentPartyID, 0, instrumentPartyID.length, err);
	...
	public void setInstrumentPartyID(byte[] src) {		
		if (src == null ) return;
		if (hasInstrumentPartyID()) FixUtils.fillNul(instrumentPartyID);		
	...
	public byte getInstrumentPartyIDSource() { 		
		if ( hasInstrumentPartyIDSource()) {		
			...
			instrumentPartyIDSource = FixUtils.getTagCharValue(buf, err);
	...
	public long getInstrumentPartyRole() { 		
		if ( hasInstrumentPartyRole()) {		
			...
			instrumentPartyRole = FixUtils.getTagIntValue(buf, err);
	...
	**/
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < instrumentPtysSubGrp.length; i++)
			if (!instrumentPtysSubGrp[i].equals(msg.instrumentPtysSubGrp[i])) return false;
