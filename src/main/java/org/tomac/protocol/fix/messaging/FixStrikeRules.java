...
public class FixStrikeRules extends FixGroup {
	...
	public FixStrikeRules(boolean isRequired) {
        			...
        			if ( tag == FixTags.NOMATURITYRULES_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !maturityRules[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOMATURITYRULES_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public void encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(maturityRules)>0) {
			out.put(FixTags.NOMATURITYRULES);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(maturityRules));

			out.put(FixUtils.SOH);

		}
		for (FixMaturityRules fixMaturityRules : maturityRules) if (fixMaturityRules.hasGroup()) fixMaturityRules.encode(out);
	}
