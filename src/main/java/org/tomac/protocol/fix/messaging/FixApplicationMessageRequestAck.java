...
public class FixApplicationMessageRequestAck extends FixInMessage {
	...
	FixApplIDRequestAckGrp[] applIDRequestAckGrp;
	FixParties[] parties;
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasApplResponseID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ApplResponseID missing", FixTags.APPLRESPONSEID_INT, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREQUESTACK);
			return false;
		}
		return true;
	}
	...
	public void setApplResponseID(byte[] src) {		
		if (src == null ) return;
		if (hasApplResponseID()) FixUtils.fillSpace(applResponseID);		
