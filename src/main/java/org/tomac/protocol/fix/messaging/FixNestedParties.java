...
public class FixNestedParties extends FixGroup {
	...
	public FixNestedParties(boolean isRequired) {
        			...
        			if ( tag == FixTags.NONESTEDPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !nstdPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NONESTEDPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public void encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(nstdPtysSubGrp)>0) {
			out.put(FixTags.NONESTEDPARTYSUBIDS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(nstdPtysSubGrp));

			out.put(FixUtils.SOH);

		}
		for (FixNstdPtysSubGrp fixNstdPtysSubGrp : nstdPtysSubGrp) if (fixNstdPtysSubGrp.hasGroup()) fixNstdPtysSubGrp.encode(out);
	}
	...
	**/
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < nstdPtysSubGrp.length; i++)
			if (!nstdPtysSubGrp[i].equals(msg.nstdPtysSubGrp[i])) return false;
