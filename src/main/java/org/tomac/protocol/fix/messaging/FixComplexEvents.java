        			...
        			if ( tag == FixTags.NOCOMPLEXEVENTDATES_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !complexEventDates[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOCOMPLEXEVENTDATES_INT);
        						return repeatingGroupTag;
        					}
        					count++;
