...
public class FixNestedParties3 extends FixGroup {
        			...
        			if ( tag == FixTags.NONESTED3PARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !nstdPtys3SubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NONESTED3PARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getNested3PartyID() { 		
		if ( hasNested3PartyID()) {		
			...
			FixUtils.getTagStringValue(buf, nested3PartyID, 0, nested3PartyID.length, err);
	...
	public byte getNested3PartyIDSource() { 		
		if ( hasNested3PartyIDSource()) {		
			...
			nested3PartyIDSource = FixUtils.getTagCharValue(buf, err);
	...
	public long getNested3PartyRole() { 		
		if ( hasNested3PartyRole()) {		
			...
			nested3PartyRole = FixUtils.getTagIntValue(buf, err);
