...
public class FixTradingSessionList extends FixInMessage {
	...
	public FixTradingSessionList() {
		super(FixMessageInfo.MessageTypes.TRADINGSESSIONLIST);
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.TRADINGSESSIONLIST_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		for (int i = 0; i< FixUtils.FIX_MAX_NOINGROUP; i++) { if (trdSessLstGrp[i].hasGroup()) trdSessLstGrp[i].hasRequiredTags(err); if (err.hasError()) return false; }
		return true;
	}
	@Override		
	public void getAll() {		
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(trdSessLstGrp)>0) {
			out.put(FixTags.NOTRADINGSESSIONS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(trdSessLstGrp));

			out.put(FixUtils.SOH);

		}
		for (FixTrdSessLstGrp fixTrdSessLstGrp : trdSessLstGrp) if (fixTrdSessLstGrp.hasGroup()) fixTrdSessLstGrp.encode(out);
	...
	**/
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixTradingSessionList)) return false;

		FixTradingSessionList msg = (FixTradingSessionList) o;
	...
	@Override
	public FixTradingSessionList clone () {
		FixTradingSessionList out = new FixTradingSessionList();
	...
	public FixTradingSessionList clone ( FixTradingSessionList out ) {
		if ( hasTradSesReqID())
