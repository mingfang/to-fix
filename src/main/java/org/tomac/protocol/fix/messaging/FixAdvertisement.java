...
public class FixAdvertisement extends FixInMessage {
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.ADVERTISEMENT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasAdvId()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.ADVID_INT, FixMessageInfo.MessageTypes.ADVERTISEMENT_INT);
			return false;
		}
		if (!hasAdvTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.ADVTRANSTYPE_INT, FixMessageInfo.MessageTypes.ADVERTISEMENT_INT);
			return false;
		}
		if (!hasAdvSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.ADVSIDE_INT, FixMessageInfo.MessageTypes.ADVERTISEMENT_INT);
			return false;
		}
		if (!hasQuantity()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.QUANTITY_INT, FixMessageInfo.MessageTypes.ADVERTISEMENT_INT);
			return false;
		}
	...
	public byte[] getAdvId() { 		
		if ( hasAdvId()) {		
			...
			FixUtils.getTagStringValue(buf, advId, 0, advId.length, err);
	...
	public void setAdvId(byte[] src) {		
		if (src == null ) return;
		if (hasAdvId()) FixUtils.fillNul(advId);		
	...
	public byte[] getAdvTransType() { 		
		if ( hasAdvTransType()) {		
			...
			FixUtils.getTagStringValue(buf, advTransType, 0, advTransType.length, err);
	...
	public void setAdvTransType(byte[] src) {		
		if (src == null ) return;
		if (hasAdvTransType()) FixUtils.fillNul(advTransType);		
	...
	public byte[] getAdvRefID() { 		
		if ( hasAdvRefID()) {		
			...
			FixUtils.getTagStringValue(buf, advRefID, 0, advRefID.length, err);
	...
	public void setAdvRefID(byte[] src) {		
		if (src == null ) return;
		if (hasAdvRefID()) FixUtils.fillNul(advRefID);		
	...
	public byte getAdvSide() { 		
		if ( hasAdvSide()) {		
			...
			advSide = FixUtils.getTagCharValue(buf, err);
