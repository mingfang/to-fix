...
public class FixTradeEntryNotificationtoEnteringFirm extends FixInMessage {
	...
	public FixTradeEntryNotificationtoEnteringFirm() {
		super(FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasAvgPx()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag AvgPx missing", FixTags.AVGPX_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasCumQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CumQty missing", FixTags.CUMQTY_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasCurrency()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Currency missing", FixTags.CURRENCY_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasExecID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecID missing", FixTags.EXECID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasExecTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecTransType missing", FixTags.EXECTRANSTYPE_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasOrderID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderID missing", FixTags.ORDERID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasOrdStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdStatus missing", FixTags.ORDSTATUS_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasSecurityID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityID missing", FixTags.SECURITYID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Side missing", FixTags.SIDE_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasSymbol()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Symbol missing", FixTags.SYMBOL_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasText()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Text missing", FixTags.TEXT_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransactTime missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasSecurityDesc()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityDesc missing", FixTags.SECURITYDESC_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasClientID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClientID missing", FixTags.CLIENTID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasExecType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecType missing", FixTags.EXECTYPE_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasLeavesQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LeavesQty missing", FixTags.LEAVESQTY_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTransBkdTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransBkdTime missing", FixTags.TRANSBKDTIME_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTradeReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportID missing", FixTags.TRADEREPORTID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasClearingInstruction()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClearingInstruction missing", FixTags.CLEARINGINSTRUCTION_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTradeType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeType missing", FixTags.TRADETYPE_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTradeReportType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportType missing", FixTags.TRADEREPORTTYPE_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		if (!hasTrdMatchID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TrdMatchID missing", FixTags.TRDMATCHID_INT, FixMessageInfo.MessageTypes.TRADEENTRYNOTIFICATIONTOENTERINGFIRM);
			return false;
		}
		return true;
	}
	...
	**/
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixTradeEntryNotificationtoEnteringFirm)) return false;

		FixTradeEntryNotificationtoEnteringFirm msg = (FixTradeEntryNotificationtoEnteringFirm) o;
	...
	public FixTradeEntryNotificationtoEnteringFirm clone ( FixTradeEntryNotificationtoEnteringFirm out ) {
		if ( hasAvgPx())
