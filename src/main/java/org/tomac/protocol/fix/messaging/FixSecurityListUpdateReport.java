...
public class FixSecurityListUpdateReport extends FixInMessage {
	...
	public FixSecurityListUpdateReport() {
		super(FixMessageInfo.MessageTypes.SECURITYLISTUPDATEREPORT);
		...
		secLstUpdRelSymGrp = new FixSecLstUpdRelSymGrp[FixUtils.FIX_MAX_NOINGROUP];
		for (int i= 0; i<FixUtils.FIX_MAX_NOINGROUP; i++) secLstUpdRelSymGrp[i] = new FixSecLstUpdRelSymGrp();
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !secLstUpdRelSymGrp[count].isKeyTag(repeatingGroupTag) ) {
        					...
        					count++;
        					repeatingGroupTag = secLstUpdRelSymGrp[count].setBuffer( repeatingGroupTag, buf, err);	
        					if (err.hasError()) break; 		
        				}
        				if (err.hasError()) break;
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.SECURITYLISTUPDATEREPORT_INT);
                			break;
                		}
					}
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(secLstUpdRelSymGrp)>0) {
			out.put(FixTags.NORELATEDSYM);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(secLstUpdRelSymGrp));

			out.put(FixUtils.SOH);

		}
		for (FixSecLstUpdRelSymGrp fixSecLstUpdRelSymGrp : secLstUpdRelSymGrp) if (fixSecLstUpdRelSymGrp.hasGroup()) fixSecLstUpdRelSymGrp.encode(out);
	...
	**/
	...
	@Override
	public FixSecurityListUpdateReport clone () {
		FixSecurityListUpdateReport out = new FixSecurityListUpdateReport();
	...
	public FixSecurityListUpdateReport clone ( FixSecurityListUpdateReport out ) {
		if ( hasSecurityReportID())
		...
		int count = 0;
		count = 0;
		for (FixSecLstUpdRelSymGrp fixSecLstUpdRelSymGrp : secLstUpdRelSymGrp) {
			if (!fixSecLstUpdRelSymGrp.hasGroup()) continue;
			out.secLstUpdRelSymGrp[count] = fixSecLstUpdRelSymGrp.clone( out.secLstUpdRelSymGrp[count] );
			count++;
		}
		return out;
	}
