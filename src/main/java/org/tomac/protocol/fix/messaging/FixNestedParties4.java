...
public class FixNestedParties4 extends FixGroup {
	...
	long nested4PartyRole = 0;		
		FixNstdPtys4SubGrp[] nstdPtys4SubGrp;
	...
	public void encode(ByteBuffer out) {
		...
		for (FixNstdPtys4SubGrp fixNstdPtys4SubGrp : nstdPtys4SubGrp) fixNstdPtys4SubGrp.encode(out);
	}
	...
	public byte[] getNested4PartyID() { 		
		if ( hasNested4PartyID()) {		
			...
			FixMessage.getTagStringValue(buf, nested4PartyID, 0, nested4PartyID.length, err);
	...
	public void setNested4PartyID(byte[] src) {		
		if (src == null ) return;
		if (hasNested4PartyID()) FixUtils.fillSpace(nested4PartyID);		
	...
	public byte getNested4PartyIDSource() { 		
		if ( hasNested4PartyIDSource()) {		
			...
			nested4PartyIDSource = FixMessage.getTagCharValue(buf, err);
	...
	public long getNested4PartyRole() { 		
		if ( hasNested4PartyRole()) {		
			...
			nested4PartyRole = FixMessage.getTagIntValue(buf, err);
