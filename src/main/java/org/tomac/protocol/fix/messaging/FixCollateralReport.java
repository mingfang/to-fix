...
public class FixCollateralReport extends FixInMessage {
	private short hasCollRptID;
	byte[] collRptID = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
	...
	public FixCollateralReport() {
		super(FixMessageInfo.MessageTypes.COLLATERALREPORT);


		hasCollRptID = FixUtils.TAG_HAS_NO_VALUE;		
		collRptID = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
		...
		hasCollStatus = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	case FixTags.COLLRPTID_INT:		
            		hasCollRptID = (short) buf.position();		
            	...
            	case FixTags.COLLSTATUS_INT:		
            		hasCollStatus = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.COLLATERALREPORT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasCollRptID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.COLLRPTID_INT, FixMessageInfo.MessageTypes.COLLATERALREPORT_INT);
			return false;
		}
		if (!hasCollStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.COLLSTATUS_INT, FixMessageInfo.MessageTypes.COLLATERALREPORT_INT);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasCollRptID()) {		
			out.put(FixTags.COLLRPTID);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,collRptID); 		
		...
		if (hasCollStatus()) {		
			out.put(FixTags.COLLSTATUS);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out, (long)collStatus);
	...
	public void crackCollRptID() {		
		getCollRptID();		
	}		
			
	public byte[] getCollRptID() { 		
		if ( hasCollRptID()) {		
			if (hasCollRptID == FixUtils.TAG_HAS_VALUE) {		
				return collRptID; 		
			} else {

				buf.position(hasCollRptID);

			FixUtils.getTagStringValue(buf, collRptID, 0, collRptID.length, err);
			...
			hasCollRptID = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return collRptID;		
		} else {		
			return null; 		
	...
	public boolean hasCollRptID() { return hasCollRptID != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setCollRptID(byte[] src) {		
		if (src == null ) return;
		if (hasCollRptID()) FixUtils.fillNul(collRptID);		
		FixUtils.copy(collRptID, src); 		
		hasCollRptID = FixUtils.TAG_HAS_VALUE;		
	}
	...
	public long getCollApplType() { 		
		if ( hasCollApplType()) {		
			...
			collApplType = FixUtils.getTagIntValue(buf, err);
	...
	public void crackCollStatus() {		
		getCollStatus();		
	}		
			
	public long getCollStatus() { 		
		if ( hasCollStatus()) {		
			if (hasCollStatus == FixUtils.TAG_HAS_VALUE) {		
				return collStatus; 		
			} else {

				buf.position(hasCollStatus);

			collStatus = FixUtils.getTagIntValue(buf, err);
			...
			hasCollStatus = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return collStatus;		
		} else {		
			return 0; 		
	...
	public boolean hasCollStatus() { return hasCollStatus != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setCollStatus(byte[] src) {		
		if (src == null ) return;
		if (hasCollStatus()) collStatus = FixUtils.TAG_HAS_NO_VALUE;		
		collStatus = FixUtils.longValueOf(src, 0, src.length);
		hasCollStatus = FixUtils.TAG_HAS_VALUE;		
	}

	public void setCollStatus(long src) {		
		collStatus = src;
		hasCollStatus = FixUtils.TAG_HAS_VALUE;		
	}
	...
	**/
	...
	public String toString() {
		...
		if (standardHeader.hasLastMsgSeqNumProcessed()) s += "LastMsgSeqNumProcessed(369)= " + standardHeader.getLastMsgSeqNumProcessed() + "\n" ; 
					...
					if (hasCollRptID()) s += "CollRptID(908)= " + new String( FixUtils.trim(getCollRptID()) ) + "\n" ; 
		...
		if (hasCollStatus()) s += "CollStatus(910)= " + getCollStatus() + "\n" ; 
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixCollateralReport)) return false;

		FixCollateralReport msg = (FixCollateralReport) o;

		if (!standardHeader.equals(msg.standardHeader)) return false;

		if ((hasCollRptID() && !msg.hasCollRptID()) || (!hasCollRptID() && msg.hasCollRptID())) return false;
		if (!(!hasCollRptID() && !msg.hasCollRptID()) && !FixUtils.equals(getCollRptID(), msg.getCollRptID())) return false;
		...
		if ((hasCollStatus() && !msg.hasCollStatus()) || (!hasCollStatus() && msg.hasCollStatus())) return false;
		if (!(!hasCollStatus() && !msg.hasCollStatus()) && !(getCollStatus()==msg.getCollStatus())) return false;
	...
	@Override
	public FixCollateralReport clone () {
		FixCollateralReport out = new FixCollateralReport();
	...
	public FixCollateralReport clone ( FixCollateralReport out ) {
		if ( hasCollRptID())
			out.setCollRptID(getCollRptID());
		if ( hasCollInquiryID())
		...
		if ( hasCollStatus())
			out.setCollStatus(getCollStatus());
		if ( hasTotNumReports())
