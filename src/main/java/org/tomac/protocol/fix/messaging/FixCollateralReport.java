...
public class FixCollateralReport extends FixInMessage {
	...
	public byte[] getCollRptID() { 		
		if ( hasCollRptID()) {		
			...
			FixMessage.getTagStringValue(buf, collRptID, 0, collRptID.length, err);
	...
	public void setCollRptID(byte[] src) {		
		if (src == null ) return;
		if (hasCollRptID()) FixUtils.fillSpace(collRptID);		
	...
	public byte[] getCollInquiryID() { 		
		if ( hasCollInquiryID()) {		
			...
			FixMessage.getTagStringValue(buf, collInquiryID, 0, collInquiryID.length, err);
	...
	public void setCollInquiryID(byte[] src) {		
		if (src == null ) return;
		if (hasCollInquiryID()) FixUtils.fillSpace(collInquiryID);		
	...
	public long getCollStatus() { 		
		if ( hasCollStatus()) {		
			...
			collStatus = FixMessage.getTagIntValue(buf, err);
