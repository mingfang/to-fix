...
public class FixSettlPtysSubGrp extends FixGroup {
	...
	public FixSettlPtysSubGrp(boolean isRequired) {
	...
	public byte[] getSettlPartySubID() { 		
		if ( hasSettlPartySubID()) {		
			...
			FixUtils.getTagStringValue(buf, settlPartySubID, 0, settlPartySubID.length, err);
	...
	public void setSettlPartySubID(byte[] src) {		
		if (src == null ) return;
		if (hasSettlPartySubID()) FixUtils.fillNul(settlPartySubID);		
	...
	public long getSettlPartySubIDType() { 		
		if ( hasSettlPartySubIDType()) {		
			...
			settlPartySubIDType = FixUtils.getTagIntValue(buf, err);
