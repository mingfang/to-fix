	...
	public void getAllGroup(ByteBuffer buf) throws FixSessionException, FixGarbledException
	{
		...
		int lastTagPosition = buf.position();
			...
			if(id == FixTags.LISTSEQNO_INT) {
				listSeqNo = FixUtils.getTagIntValue(null ,id ,value );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.SETTLINSTMODE_INT) {
				settlInstMode = FixUtils.getTagCharValue(null ,id ,value );
				if (!FixMessageInfo.SettlInstMode.isValid(settlInstMode) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + settlInstMode + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.ACCOUNT_INT) {
				account = FixUtils.getTagStringValue(null ,id ,value, account);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.ACCTIDSOURCE_INT) {
				acctIDSource = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.ACCOUNTTYPE_INT) {
				accountType = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.DAYBOOKINGINST_INT) {
				dayBookingInst = FixUtils.getTagCharValue(null ,id ,value );
			...
			if(id == FixTags.BOOKINGUNIT_INT) {
				bookingUnit = FixUtils.getTagCharValue(null ,id ,value );
			...
			if(id == FixTags.ALLOCID_INT) {
				allocID = FixUtils.getTagStringValue(null ,id ,value, allocID);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.PREALLOCMETHOD_INT) {
				preallocMethod = FixUtils.getTagCharValue(null ,id ,value );
			...
			if(id == FixTags.NOALLOCS_INT) {
				int noAllocs;
				noAllocs = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.CASHMARGIN_INT) {
				cashMargin = FixUtils.getTagCharValue(null ,id ,value );
			...
			if(id == FixTags.HANDLINST_INT) {
				handlInst = FixUtils.getTagCharValue(null ,id ,value );
				if (!FixMessageInfo.HandlInst.isValid(handlInst) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + handlInst + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.MINQTY_INT) {
				minQty = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.MATCHINCREMENT_INT) {
				matchIncrement = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.MAXPRICELEVELS_INT) {
				maxPriceLevels = FixUtils.getTagIntValue(null ,id ,value );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.MAXFLOOR_INT) {
				maxFloor = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.EXDESTINATION_INT) {
				exDestination = FixUtils.getTagStringValue(null ,id ,value, exDestination);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.EXDESTINATIONIDSOURCE_INT) {
				exDestinationIDSource = FixUtils.getTagCharValue(null ,id ,value );
				if (!FixMessageInfo.ExDestinationIDSource.isValid(exDestinationIDSource) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + exDestinationIDSource + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.NOTRADINGSESSIONS_INT) {
				...
				noTradingSessions = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.PREVCLOSEPX_INT) {
				prevClosePx = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.LOCATEREQD_INT) {
				locateReqd = FixUtils.getTagBooleanValue(null ,id ,value );
				if (!FixMessageInfo.LocateReqd.isValid(locateReqd) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + locateReqd + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.QTYTYPE_INT) {
				qtyType = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.PRICEPROTECTIONSCOPE_INT) {
				priceProtectionScope = FixUtils.getTagCharValue(null ,id ,value );
				if (!FixMessageInfo.PriceProtectionScope.isValid(priceProtectionScope) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + priceProtectionScope + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.SOLICITEDFLAG_INT) {
				solicitedFlag = FixUtils.getTagBooleanValue(null ,id ,value );
			...
			if(id == FixTags.IOIID_INT) {
				iOIID = FixUtils.getTagStringValue(null ,id ,value, iOIID);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.QUOTEID_INT) {
				quoteID = FixUtils.getTagStringValue(null ,id ,value, quoteID);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.REFORDERIDSOURCE_INT) {
				...
				if (!FixMessageInfo.RefOrderIDSource.isValid(refOrderIDSource) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + refOrderIDSource + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.EXPIREDATE_INT) {
				expireDate = FixUtils.getTagStringValue(null ,id ,value, expireDate);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.GTBOOKINGINST_INT) {
				gTBookingInst = FixUtils.getTagIntValue(null ,id ,value );
				if (!FixMessageInfo.GTBookingInst.isValid(gTBookingInst) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + gTBookingInst + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.PRETRADEANONYMITY_INT) {
				preTradeAnonymity = FixUtils.getTagBooleanValue(null ,id ,value );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.CUSTORDERCAPACITY_INT) {
				custOrderCapacity = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.FOREXREQ_INT) {
				forexReq = FixUtils.getTagBooleanValue(null ,id ,value );
			...
			if(id == FixTags.SETTLDATE2_INT) {
				settlDate2 = FixUtils.getTagStringValue(null ,id ,value, settlDate2);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.ORDERQTY2_INT) {
				orderQty2 = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.PRICE2_INT) {
				price2 = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.POSITIONEFFECT_INT) {
				positionEffect = FixUtils.getTagCharValue(null ,id ,value );
			...
			if(id == FixTags.COVEREDORUNCOVERED_INT) {
				coveredOrUncovered = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.MAXSHOW_INT) {
				maxShow = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.TARGETSTRATEGY_INT) {
				targetStrategy = FixUtils.getTagIntValue(null ,id ,value );
				if (!FixMessageInfo.TargetStrategy.isValid(targetStrategy) ) throw new FixSessionException(SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG, ("Invalid enumerated value(" + targetStrategy + ") for tag").getBytes(), id, new byte[0] );
				lastTagPosition = buf.position();
			...
			if(id == FixTags.NOSTRATEGYPARAMETERS_INT) {
				...
				noStrategyParameters = FixUtils.getTagIntValue(null ,id ,value );
			...
			if(id == FixTags.TARGETSTRATEGYPARAMETERS_INT) {
				targetStrategyParameters = FixUtils.getTagStringValue(null ,id ,value, targetStrategyParameters);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.PARTICIPATIONRATE_INT) {
				participationRate = FixUtils.getTagFloatValue(null ,id ,value);
				lastTagPosition = buf.position();
			...
			if(id == FixTags.DESIGNATION_INT) {
				designation = FixUtils.getTagStringValue(null ,id ,value, designation);
				lastTagPosition = buf.position();
