...
public class FixUndSecAltIDGrp extends FixGroup {
	...
	public byte[] getUnderlyingSecurityAltID() { 		
		if ( hasUnderlyingSecurityAltID()) {		
			...
			FixMessage.getTagStringValue(buf, underlyingSecurityAltID, 0, underlyingSecurityAltID.length, err);
	...
	public void setUnderlyingSecurityAltID(byte[] src) {		
		if (src == null ) return;
		if (hasUnderlyingSecurityAltID()) FixUtils.fillSpace(underlyingSecurityAltID);		
	...
	public byte[] getUnderlyingSecurityAltIDSource() { 		
		if ( hasUnderlyingSecurityAltIDSource()) {		
			...
			FixMessage.getTagStringValue(buf, underlyingSecurityAltIDSource, 0, underlyingSecurityAltIDSource.length, err);
	...
	public void setUnderlyingSecurityAltIDSource(byte[] src) {		
		if (src == null ) return;
		if (hasUnderlyingSecurityAltIDSource()) FixUtils.fillSpace(underlyingSecurityAltIDSource);		
