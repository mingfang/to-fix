...
public class FixPositionReport extends FixInMessage {
	...
	private short hasDeliveryDate;
	byte[] deliveryDate = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
	private short hasModelType;
	long modelType = 0;		
	...
	public FixPositionReport() {
		...
		hasDeliveryDate = FixUtils.TAG_HAS_NO_VALUE;		
		deliveryDate = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
		hasModelType = FixUtils.TAG_HAS_NO_VALUE;		
		...
		posUndInstrmtGrp = new FixPosUndInstrmtGrp[FixUtils.FIX_MAX_NOINGROUP];
		for (int i= 0; i<FixUtils.FIX_MAX_NOINGROUP; i++) posUndInstrmtGrp[i] = new FixPosUndInstrmtGrp();
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.REGISTSTATUS_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break;
            	case FixTags.DELIVERYDATE_INT:		
            		hasDeliveryDate = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
            	case FixTags.MODELTYPE_INT:		
            		hasModelType = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.POSITIONREPORT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasPosMaintRptID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.POSMAINTRPTID_INT, FixMessageInfo.MessageTypes.POSITIONREPORT_INT);
			return false;
		}
		if (!hasClearingBusinessDate()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.CLEARINGBUSINESSDATE_INT, FixMessageInfo.MessageTypes.POSITIONREPORT_INT);
			return false;
		}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasDeliveryDate()) {		
			out.put(FixTags.DELIVERYDATE);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,deliveryDate); 		
		
			out.put(FixUtils.SOH);		
		}		
		if (hasModelType()) {		
			out.put(FixTags.MODELTYPE);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out, (long)modelType);
	...
	public long getSettlPrice() { 		
		if ( hasSettlPrice()) {		
			...
			settlPrice = FixUtils.getTagFloatValue(buf, err);
	...
	public long getSettlPriceType() { 		
		if ( hasSettlPriceType()) {		
			...
			settlPriceType = FixUtils.getTagIntValue(buf, err);
	...
	public long getPriorSettlPrice() { 		
		if ( hasPriorSettlPrice()) {		
			...
			priorSettlPrice = FixUtils.getTagFloatValue(buf, err);
	...
	public void crackDeliveryDate() {		
		getDeliveryDate();		
	}		
			
	public byte[] getDeliveryDate() { 		
		if ( hasDeliveryDate()) {		
			if (hasDeliveryDate == FixUtils.TAG_HAS_VALUE) {		
				return deliveryDate; 		
			} else {

				buf.position(hasDeliveryDate);

			FixUtils.getTagStringValue(buf, deliveryDate, 0, deliveryDate.length, err);
			...
			hasDeliveryDate = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return deliveryDate;		
		} else {		
			return null; 		
	...
	public boolean hasDeliveryDate() { return hasDeliveryDate != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setDeliveryDate(byte[] src) {		
		if (src == null ) return;
		if (hasDeliveryDate()) FixUtils.fillNul(deliveryDate);		
		FixUtils.copy(deliveryDate, src); 		
		hasDeliveryDate = FixUtils.TAG_HAS_VALUE;		
	}
	...
	public void crackModelType() {		
		getModelType();		
	}		
			
	public long getModelType() { 		
		if ( hasModelType()) {		
			if (hasModelType == FixUtils.TAG_HAS_VALUE) {		
				return modelType; 		
			} else {

				buf.position(hasModelType);

			modelType = FixUtils.getTagIntValue(buf, err);
			...
			hasModelType = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return modelType;		
		} else {		
			return 0; 		
	...
	public boolean hasModelType() { return hasModelType != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setModelType(byte[] src) {		
		if (src == null ) return;
		if (hasModelType()) modelType = FixUtils.TAG_HAS_NO_VALUE;		
		modelType = FixUtils.longValueOf(src, 0, src.length);
		hasModelType = FixUtils.TAG_HAS_VALUE;		
	}

	public void setModelType(long src) {		
		modelType = src;
		hasModelType = FixUtils.TAG_HAS_VALUE;		
	}
	...
	**/
	...
	public String toString() {
		...
		if (hasDeliveryDate()) s += "DeliveryDate(743)= " + new String( FixUtils.trim(getDeliveryDate()) ) + "\n" ; 
		if (hasModelType()) s += "ModelType(1434)= " + getModelType() + "\n" ; 
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixPositionReport)) return false;

		FixPositionReport msg = (FixPositionReport) o;
	...
	@Override
	public FixPositionReport clone () {
		FixPositionReport out = new FixPositionReport();
	...
	public FixPositionReport clone ( FixPositionReport out ) {
		if ( hasPosMaintRptID())
		...
		if ( hasDeliveryDate())
			out.setDeliveryDate(getDeliveryDate());
		if ( hasModelType())
			out.setModelType(getModelType());
		if ( hasPriceDelta())
		...
		count = 0;
		for (FixPosUndInstrmtGrp fixPosUndInstrmtGrp : posUndInstrmtGrp) {
			if (!fixPosUndInstrmtGrp.hasGroup()) continue;
			out.posUndInstrmtGrp[count] = fixPosUndInstrmtGrp.clone( out.posUndInstrmtGrp[count] );
			count++;
		}
		count = 0;
