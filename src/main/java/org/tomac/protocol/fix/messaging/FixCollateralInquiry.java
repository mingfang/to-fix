...
public class FixCollateralInquiry extends FixInMessage {
	...
	FixCollInqQualGrp[] collInqQualGrp;
	FixParties[] parties;
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasCollInquiryID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CollInquiryID missing", FixTags.COLLINQUIRYID_INT, FixMessageInfo.MessageTypes.COLLATERALINQUIRY);
			return false;
		}
		return true;
	}
