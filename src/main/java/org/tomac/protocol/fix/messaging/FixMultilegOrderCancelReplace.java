...
public class FixMultilegOrderCancelReplace extends FixInMessage {
	...
	public FixMultilegOrderCancelReplace() {
		super(FixMessageInfo.MessageTypes.MULTILEGORDERCANCELREPLACE);
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Side missing", FixTags.SIDE_INT, FixMessageInfo.MessageTypes.MULTILEGORDERCANCELREPLACE);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransactTime missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.MULTILEGORDERCANCELREPLACE);
			return false;
		}
		if (!hasOrdType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdType missing", FixTags.ORDTYPE_INT, FixMessageInfo.MessageTypes.MULTILEGORDERCANCELREPLACE);
			return false;
		}
		return true;
	}
	...
	public long getSwapPoints() { 		
		if ( hasSwapPoints()) {		
			...
			swapPoints = FixMessage.getTagFloatValue(buf, err);
	...
	public long getMultiLegRptTypeReq() { 		
		if ( hasMultiLegRptTypeReq()) {		
			...
			multiLegRptTypeReq = FixMessage.getTagIntValue(buf, err);
	...
	**/
	...
	@Override
	public FixMultilegOrderCancelReplace clone () {
		FixMultilegOrderCancelReplace out = new FixMultilegOrderCancelReplace();
	...
	public FixMultilegOrderCancelReplace clone ( FixMultilegOrderCancelReplace out ) {
		if ( hasOrderID())
