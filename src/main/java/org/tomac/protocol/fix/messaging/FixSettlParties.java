...
public class FixSettlParties extends FixGroup {
        			...
        			if ( tag == FixTags.NOSETTLPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !settlPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOSETTLPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getSettlPartyID() { 		
		if ( hasSettlPartyID()) {		
			...
			FixUtils.getTagStringValue(buf, settlPartyID, 0, settlPartyID.length, err);
	...
	public byte getSettlPartyIDSource() { 		
		if ( hasSettlPartyIDSource()) {		
			...
			settlPartyIDSource = FixUtils.getTagCharValue(buf, err);
	...
	public long getSettlPartyRole() { 		
		if ( hasSettlPartyRole()) {		
			...
			settlPartyRole = FixUtils.getTagIntValue(buf, err);
