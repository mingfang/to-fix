        			...
        			if ( underlyingInstrument.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !underlyingAmount[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDERLYINGAMOUNTS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
