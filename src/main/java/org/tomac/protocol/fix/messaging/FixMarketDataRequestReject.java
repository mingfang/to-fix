...
public class FixMarketDataRequestReject extends FixInMessage {
        			...
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !mDRjctGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOALTMDSOURCE_INT);
        						return;
        					}
        					count++;
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.MARKETDATAREQUESTREJECT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasMDReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.MDREQID_INT, FixMessageInfo.MessageTypes.MARKETDATAREQUESTREJECT_INT);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(mDRjctGrp)>0) {
			out.put(FixTags.NOALTMDSOURCE);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(mDRjctGrp));

			out.put(FixUtils.SOH);

		}
		for (FixMDRjctGrp fixMDRjctGrp : mDRjctGrp) if (fixMDRjctGrp.hasGroup()) fixMDRjctGrp.encode(out);
	...
	public byte getMDReqRejReason() { 		
		if ( hasMDReqRejReason()) {		
			...
			mDReqRejReason = FixUtils.getTagCharValue(buf, err);
