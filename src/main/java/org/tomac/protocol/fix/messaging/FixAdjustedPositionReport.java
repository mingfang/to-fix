...
public class FixAdjustedPositionReport extends FixInMessage {
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasPosMaintRptID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag PosMaintRptID missing", FixTags.POSMAINTRPTID_INT, FixMessageInfo.MessageTypes.ADJUSTEDPOSITIONREPORT);
			return false;
		}
		if (!hasClearingBusinessDate()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClearingBusinessDate missing", FixTags.CLEARINGBUSINESSDATE_INT, FixMessageInfo.MessageTypes.ADJUSTEDPOSITIONREPORT);
			return false;
		}
		return true;
	}
