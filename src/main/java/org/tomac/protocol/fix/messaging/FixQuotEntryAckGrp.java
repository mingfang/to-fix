...
public class FixQuotEntryAckGrp extends FixGroup {
	...
	private short hasQuoteEntryStatus;
	long quoteEntryStatus = 0;		
	...
	private short hasQuoteEntryRejectReason;
	long quoteEntryRejectReason = 0;		
		public FixInstrument instrument;
		public FixInstrmtLegGrp[] instrmtLegGrp;
	
	public FixQuotEntryAckGrp() {
		this(false);
	}

	public FixQuotEntryAckGrp(boolean isRequired) {
		...
		hasQuoteEntryStatus = FixUtils.TAG_HAS_NO_VALUE;		
		...
		hasQuoteEntryRejectReason = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.CURRENCY_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break; 		
            	case FixTags.QUOTEENTRYSTATUS_INT:		
            		hasQuoteEntryStatus = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break; 		
            	...
            	case FixTags.ORDERRESTRICTIONS_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break; 		
            	case FixTags.QUOTEENTRYREJECTREASON_INT:		
            		hasQuoteEntryRejectReason = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break; 		
            	default:
	...
	public void encode(ByteBuffer out) {
		...
		if (hasQuoteEntryStatus()) {		
			out.put(FixTags.QUOTEENTRYSTATUS);

			out.put((byte) '=');

			FixUtils.put(out, (long)quoteEntryStatus);
		...
		if (hasQuoteEntryRejectReason()) {		
			out.put(FixTags.QUOTEENTRYREJECTREASON);

			out.put((byte) '=');

			FixUtils.put(out, (long)quoteEntryRejectReason);
	...
	public long getQuoteEntryStatus() { 		
		if ( hasQuoteEntryStatus()) {		
			if (hasQuoteEntryStatus == FixUtils.TAG_HAS_VALUE) {		
				return quoteEntryStatus; 		
			} else {		
		
				buf.position(hasQuoteEntryStatus);		
		
			quoteEntryStatus = FixUtils.getTagIntValue(buf, err);
			...
			hasQuoteEntryStatus = FixUtils.TAG_HAS_VALUE;		
			return quoteEntryStatus;		
		} else {		
			return 0; 		
		}		
	...
	public boolean hasQuoteEntryStatus() { return hasQuoteEntryStatus != FixUtils.TAG_HAS_NO_VALUE; } 		
		
	public void setQuoteEntryStatus(long src) {		
		quoteEntryStatus = src;
		hasQuoteEntryStatus = FixUtils.TAG_HAS_VALUE;		
	}

	public void setQuoteEntryStatus(byte[] src) {		
		if (src == null ) return;
		if (hasQuoteEntryStatus()) quoteEntryStatus = FixUtils.TAG_HAS_NO_VALUE;		
		quoteEntryStatus = FixUtils.longValueOf(src, 0, src.length);
		hasQuoteEntryStatus = FixUtils.TAG_HAS_VALUE;		
	}		
			
	public void setQuoteEntryStatus(String str) {		
		if (str == null ) return;
		if (hasQuoteEntryStatus()) quoteEntryStatus = FixUtils.TAG_HAS_NO_VALUE;		
		byte[] src = str.getBytes(); 		
		quoteEntryStatus = FixUtils.longValueOf(src, 0, src.length);
	...
	public long getQuoteEntryRejectReason() { 		
		if ( hasQuoteEntryRejectReason()) {		
			if (hasQuoteEntryRejectReason == FixUtils.TAG_HAS_VALUE) {		
				return quoteEntryRejectReason; 		
			} else {		
		
				buf.position(hasQuoteEntryRejectReason);		
		
			quoteEntryRejectReason = FixUtils.getTagIntValue(buf, err);
			...
			hasQuoteEntryRejectReason = FixUtils.TAG_HAS_VALUE;		
			return quoteEntryRejectReason;		
		} else {		
			return 0; 		
		}		
	...
	public boolean hasQuoteEntryRejectReason() { return hasQuoteEntryRejectReason != FixUtils.TAG_HAS_NO_VALUE; } 		
		
	public void setQuoteEntryRejectReason(long src) {		
		quoteEntryRejectReason = src;
		hasQuoteEntryRejectReason = FixUtils.TAG_HAS_VALUE;		
	}

	public void setQuoteEntryRejectReason(byte[] src) {		
		if (src == null ) return;
		if (hasQuoteEntryRejectReason()) quoteEntryRejectReason = FixUtils.TAG_HAS_NO_VALUE;		
		quoteEntryRejectReason = FixUtils.longValueOf(src, 0, src.length);
		hasQuoteEntryRejectReason = FixUtils.TAG_HAS_VALUE;		
	}		
			
	public void setQuoteEntryRejectReason(String str) {		
		if (str == null ) return;
		if (hasQuoteEntryRejectReason()) quoteEntryRejectReason = FixUtils.TAG_HAS_NO_VALUE;		
		byte[] src = str.getBytes(); 		
		quoteEntryRejectReason = FixUtils.longValueOf(src, 0, src.length);
	...
	**/
	...
	public String toString() {
		...
		if (hasQuoteEntryStatus()) s += "QuoteEntryStatus(1167)= " + getQuoteEntryStatus() + "\n" ; 
		...
		if (hasQuoteEntryRejectReason()) s += "QuoteEntryRejectReason(368)= " + getQuoteEntryRejectReason() + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if ((hasQuoteEntryStatus() && !msg.hasQuoteEntryStatus()) || (!hasQuoteEntryStatus() && msg.hasQuoteEntryStatus())) return false;
		if (!(!hasQuoteEntryStatus() && !msg.hasQuoteEntryStatus()) && !(getQuoteEntryStatus()==msg.getQuoteEntryStatus())) return false;
		...
		if ((hasQuoteEntryRejectReason() && !msg.hasQuoteEntryRejectReason()) || (!hasQuoteEntryRejectReason() && msg.hasQuoteEntryRejectReason())) return false;
		if (!(!hasQuoteEntryRejectReason() && !msg.hasQuoteEntryRejectReason()) && !(getQuoteEntryRejectReason()==msg.getQuoteEntryRejectReason())) return false;
		return true;
	}
	public FixQuotEntryAckGrp clone ( FixQuotEntryAckGrp out ) {
		if ( hasQuoteEntryID())
		...
		if ( hasQuoteEntryStatus())
			out.setQuoteEntryStatus(getQuoteEntryStatus());
		if ( hasBookingType())
		...
		if ( hasQuoteEntryRejectReason())
			out.setQuoteEntryRejectReason(getQuoteEntryRejectReason());
