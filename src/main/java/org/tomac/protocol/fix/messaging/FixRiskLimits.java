...
public class FixRiskLimits extends FixGroup {
	...
	public FixRiskLimits(boolean isRequired) {
	...
	public void encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(riskInstrumentScope)>0) {
			out.put(FixTags.NORISKINSTRUMENTS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(riskInstrumentScope));

			out.put(FixUtils.SOH);

		}
		for (FixRiskInstrumentScope fixRiskInstrumentScope : riskInstrumentScope) if (fixRiskInstrumentScope.hasGroup()) fixRiskInstrumentScope.encode(out);
		if (FixUtils.getNoInGroup(riskWarningLevels)>0) {
			out.put(FixTags.NORISKWARNINGLEVELS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(riskWarningLevels));

			out.put(FixUtils.SOH);

		}
		for (FixRiskWarningLevels fixRiskWarningLevels : riskWarningLevels) if (fixRiskWarningLevels.hasGroup()) fixRiskWarningLevels.encode(out);
	}
