...
public class FixExecutionReportFill extends FixInMessage {
	...
	byte[] tradeDate = new byte[FixUtils.UTCTIMESTAMP_LENGTH];		
	...
	private short hasLiquidityFlag;
	byte liquidityFlag = (byte)' ';		
	
	public FixExecutionReportFill() {
		super(FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
		...
		tradeDate = new byte[FixUtils.UTCTIMESTAMP_LENGTH];		
		...
		hasLiquidityFlag = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.BRSEQNBR_INT:		
            		...
            		FixMessage.getNext(buf, err);		
                	break;
            	case FixTags.LIQUIDITYFLAG_INT:		
            		hasLiquidityFlag = (short) buf.position();		
            		FixMessage.getNext(buf, err);		
                	break;
            	default:
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasAvgPx()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag AvgPx missing", FixTags.AVGPX_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasClOrdID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClOrdID missing", FixTags.CLORDID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasCumQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CumQty missing", FixTags.CUMQTY_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasCurrency()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Currency missing", FixTags.CURRENCY_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasExecID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecID missing", FixTags.EXECID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasExecTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecTransType missing", FixTags.EXECTRANSTYPE_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasLastPx()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LastPx missing", FixTags.LASTPX_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasLastShares()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LastShares missing", FixTags.LASTSHARES_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasOrderID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderID missing", FixTags.ORDERID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasOrderQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderQty missing", FixTags.ORDERQTY_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasOrdStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdStatus missing", FixTags.ORDSTATUS_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasOrdType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdType missing", FixTags.ORDTYPE_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasSecurityID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityID missing", FixTags.SECURITYID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Side missing", FixTags.SIDE_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasSymbol()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Symbol missing", FixTags.SYMBOL_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasClientID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClientID missing", FixTags.CLIENTID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasExecType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecType missing", FixTags.EXECTYPE_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasLeavesQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LeavesQty missing", FixTags.LEAVESQTY_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasOrderCapacity()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderCapacity missing", FixTags.ORDERCAPACITY_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasSubMktID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SubMktID missing", FixTags.SUBMKTID_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		if (!hasLiquidityFlag()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LiquidityFlag missing", FixTags.LIQUIDITYFLAG_INT, FixMessageInfo.MessageTypes.EXECUTIONREPORTFILL);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasLiquidityFlag()) {		
			out.put(FixTags.LIQUIDITYFLAG);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,liquidityFlag); 		
	...
	public void crackLiquidityFlag() {		
		getLiquidityFlag();		
	}		
			
	public byte getLiquidityFlag() { 		
		if ( hasLiquidityFlag()) {		
			if (hasLiquidityFlag == FixUtils.TAG_HAS_VALUE) {		
				return liquidityFlag; 		
			} else {

				buf.position(hasLiquidityFlag);

			liquidityFlag = FixMessage.getTagCharValue(buf, err);
			if( !err.hasError() && (liquidityFlag != (byte)'3') && (liquidityFlag != (byte)'2') && (liquidityFlag != (byte)'E') && (liquidityFlag != (byte)'1') && (liquidityFlag != (byte)'A') && (liquidityFlag != (byte)'C') && (liquidityFlag != (byte)'R') && (liquidityFlag != (byte)'N') && (liquidityFlag != (byte)'Y') && (liquidityFlag != (byte)'X') && true)
				err.setError((int)FixMessageInfo.SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG,
					"Tag msgType missing got " + 9882);		
				if (err.hasError()) {		
					buf.position(0);		
					return (byte)'0';		
				}		
			}		
			hasLiquidityFlag = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return liquidityFlag;		
		} else {		
			return (byte)'0'; 		
	...
	public boolean hasLiquidityFlag() { return hasLiquidityFlag != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setLiquidityFlag(byte[] src) {		
		if (src == null ) return;
		if (hasLiquidityFlag()) liquidityFlag = (byte)' ';		
		liquidityFlag = src[0];		
		hasLiquidityFlag = FixUtils.TAG_HAS_VALUE;		
	}
	...
	public void setLiquidityFlag(String str) {		
		if (str == null ) return;
		if (hasLiquidityFlag()) liquidityFlag = (byte)' ';		
		byte[] src = str.getBytes(); 		
		liquidityFlag = src[0];		
	...
	**/
	...
	public String toString() {
		...
		if (hasLiquidityFlag()) s += "LiquidityFlag(9882)= " + getLiquidityFlag() + "\n" ; 
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixExecutionReportFill)) return false;

		FixExecutionReportFill msg = (FixExecutionReportFill) o;
		...
		if ((hasLiquidityFlag() && !msg.hasLiquidityFlag()) || (!hasLiquidityFlag() && msg.hasLiquidityFlag())) return false;
		if (!(!hasLiquidityFlag() && !msg.hasLiquidityFlag()) && !(getLiquidityFlag()==msg.getLiquidityFlag())) return false;
		return true;
	}
	@Override
	...
	public FixExecutionReportFill clone ( FixExecutionReportFill out ) {
		if ( hasAvgPx())
		...
		if ( hasLiquidityFlag())
			out.setLiquidityFlag(getLiquidityFlag());
		return out;
	}
