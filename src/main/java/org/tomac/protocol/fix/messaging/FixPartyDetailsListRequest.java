...
public class FixPartyDetailsListRequest extends FixInMessage {
        			...
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !partyListResponseTypeGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOPARTYLISTRESPONSETYPES_INT);
        						return;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !requestedPartyRoleGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOREQUESTEDPARTYROLES_INT);
        						return;
        					}
        					count++;
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.PARTYDETAILSLISTREQUEST_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasPartyDetailsListRequestID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.PARTYDETAILSLISTREQUESTID_INT, FixMessageInfo.MessageTypes.PARTYDETAILSLISTREQUEST_INT);
			return false;
		}
