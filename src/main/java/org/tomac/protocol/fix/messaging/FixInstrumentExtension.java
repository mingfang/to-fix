        			...
        			if ( tag == FixTags.NOINSTRATTRIB_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !attrbGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOINSTRATTRIB_INT);
        						return repeatingGroupTag;
        					}
        					count++;
