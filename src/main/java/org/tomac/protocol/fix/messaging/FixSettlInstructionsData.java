        			...
        			if ( tag == FixTags.NODLVYINST_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !dlvyInstGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NODLVYINST_INT);
        						return repeatingGroupTag;
        					}
        					count++;
