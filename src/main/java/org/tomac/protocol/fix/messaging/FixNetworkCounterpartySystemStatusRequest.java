...
public class FixNetworkCounterpartySystemStatusRequest extends FixInMessage {
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasNetworkRequestType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag NetworkRequestType missing", FixTags.NETWORKREQUESTTYPE_INT, FixMessageInfo.MessageTypes.NETWORKCOUNTERPARTYSYSTEMSTATUSREQUEST);
			return false;
		}
		if (!hasNetworkRequestID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag NetworkRequestID missing", FixTags.NETWORKREQUESTID_INT, FixMessageInfo.MessageTypes.NETWORKCOUNTERPARTYSYSTEMSTATUSREQUEST);
			return false;
		}
		return true;
	}
