        			...
        			if ( tag == FixTags.NOUNDERLYINGLEGSECURITYALTID_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !underlyingLegSecurityAltIDGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDERLYINGLEGSECURITYALTID_INT);
        						return repeatingGroupTag;
        					}
        					count++;
