...
public class FixTrdCapRptSideGrp extends FixGroup {
	...
	private short hasExchangeSpecialInstructions;
	byte[] exchangeSpecialInstructions = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
	private short hasOrderCategory;
	byte orderCategory = (byte)' ';		
	private short hasSideLiquidityInd;
	long sideLiquidityInd = 0;		
	...
	public FixTrdCapRptSideGrp() {
		this(false);
	}

	public FixTrdCapRptSideGrp(boolean isRequired) {
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.AGGRESSORINDICATOR_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break; 		
            	case FixTags.EXCHANGESPECIALINSTRUCTIONS_INT:		
            		hasExchangeSpecialInstructions = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break; 		
            	case FixTags.ORDERCATEGORY_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break; 		
            	case FixTags.SIDELIQUIDITYIND_INT:		
            		hasSideLiquidityInd = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break; 		
            	default:
        			if ( tag == FixTags.NOPARTYIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !clrInstGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOCLEARINGINSTRUCTIONS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !contAmtGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOCONTAMTS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !miscFeesGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOMISCFEES_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !sideTrdRegTS[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOSIDETRDREGTS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !settlDetails[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOSETTLDETAILS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public void encode(ByteBuffer out) {
		...
		if (hasExchangeSpecialInstructions()) {		
			out.put(FixTags.EXCHANGESPECIALINSTRUCTIONS);

			out.put((byte) '=');

			FixUtils.put(out,exchangeSpecialInstructions); 		
		...
		if (hasSideLiquidityInd()) {		
			out.put(FixTags.SIDELIQUIDITYIND);

			out.put((byte) '=');

			FixUtils.put(out, (long)sideLiquidityInd);
	...
	public byte[] getSideExecID() { 		
		if ( hasSideExecID()) {		
			...
			FixUtils.getTagStringValue(buf, sideExecID, 0, sideExecID.length, err);
	...
	public long getOrderDelay() { 		
		if ( hasOrderDelay()) {		
			...
			orderDelay = FixUtils.getTagIntValue(buf, err);
	...
	public long getOrderDelayUnit() { 		
		if ( hasOrderDelayUnit()) {		
			...
			orderDelayUnit = FixUtils.getTagIntValue(buf, err);
	...
	public long getSideLastQty() { 		
		if ( hasSideLastQty()) {		
			...
			sideLastQty = FixUtils.getTagIntValue(buf, err);
	...
	public byte[] getSideTradeReportID() { 		
		if ( hasSideTradeReportID()) {		
			...
			FixUtils.getTagStringValue(buf, sideTradeReportID, 0, sideTradeReportID.length, err);
	...
	public byte[] getSideFillStationCd() { 		
		if ( hasSideFillStationCd()) {		
			...
			FixUtils.getTagStringValue(buf, sideFillStationCd, 0, sideFillStationCd.length, err);
	...
	public byte[] getSideReasonCd() { 		
		if ( hasSideReasonCd()) {		
			...
			FixUtils.getTagStringValue(buf, sideReasonCd, 0, sideReasonCd.length, err);
	...
	public long getSideTrdSubTyp() { 		
		if ( hasSideTrdSubTyp()) {		
			...
			sideTrdSubTyp = FixUtils.getTagIntValue(buf, err);
	...
	public byte[] getSideCurrency() { 		
		if ( hasSideCurrency()) {		
			...
			FixUtils.getTagStringValue(buf, sideCurrency, 0, sideCurrency.length, err);
	...
	public byte[] getSideSettlCurrency() { 		
		if ( hasSideSettlCurrency()) {		
			...
			FixUtils.getTagStringValue(buf, sideSettlCurrency, 0, sideSettlCurrency.length, err);
	...
	public boolean getOddLot() { 		
		if ( hasOddLot()) {		
			...
			oddLot = FixUtils.getTagCharValue(buf, err)=='Y'?true:false;
	...
	public long getNumDaysInterest() { 		
		if ( hasNumDaysInterest()) {		
			...
			numDaysInterest = FixUtils.getTagIntValue(buf, err);
	...
	public byte[] getExDate() { 		
		if ( hasExDate()) {		
			...
			FixUtils.getTagStringValue(buf, exDate, 0, exDate.length, err);
	...
	public long getAccruedInterestRate() { 		
		if ( hasAccruedInterestRate()) {		
			...
			accruedInterestRate = FixUtils.getTagFloatValue(buf, err);
	...
	public long getInterestAtMaturity() { 		
		if ( hasInterestAtMaturity()) {		
			...
			interestAtMaturity = FixUtils.getTagFloatValue(buf, err);
	...
	public long getConcession() { 		
		if ( hasConcession()) {		
			...
			concession = FixUtils.getTagFloatValue(buf, err);
	...
	public long getTotalTakedown() { 		
		if ( hasTotalTakedown()) {		
			...
			totalTakedown = FixUtils.getTagFloatValue(buf, err);
	...
	public long getNetMoney() { 		
		if ( hasNetMoney()) {		
			...
			netMoney = FixUtils.getTagFloatValue(buf, err);
	...
	public byte getSettlCurrFxRateCalc() { 		
		if ( hasSettlCurrFxRateCalc()) {		
			...
			settlCurrFxRateCalc = FixUtils.getTagCharValue(buf, err);
	...
	public long getSideMultiLegReportingType() { 		
		if ( hasSideMultiLegReportingType()) {		
			...
			sideMultiLegReportingType = FixUtils.getTagIntValue(buf, err);
	...
	public byte[] getExchangeRule() { 		
		if ( hasExchangeRule()) {		
			...
			FixUtils.getTagStringValue(buf, exchangeRule, 0, exchangeRule.length, err);
	...
	public long getTradeAllocIndicator() { 		
		if ( hasTradeAllocIndicator()) {		
			...
			tradeAllocIndicator = FixUtils.getTagIntValue(buf, err);
	...
	public long getSideGrossTradeAmt() { 		
		if ( hasSideGrossTradeAmt()) {		
			...
			sideGrossTradeAmt = FixUtils.getTagFloatValue(buf, err);
	...
	public boolean getAggressorIndicator() { 		
		if ( hasAggressorIndicator()) {		
			...
			aggressorIndicator = FixUtils.getTagCharValue(buf, err)=='Y'?true:false;
	...
	public byte[] getExchangeSpecialInstructions() { 		
		if ( hasExchangeSpecialInstructions()) {		
			if (hasExchangeSpecialInstructions == FixUtils.TAG_HAS_VALUE) {		
				return exchangeSpecialInstructions; 		
			} else {		
		
				buf.position(hasExchangeSpecialInstructions);		
		
			FixUtils.getTagStringValue(buf, exchangeSpecialInstructions, 0, exchangeSpecialInstructions.length, err);
			...
			hasExchangeSpecialInstructions = FixUtils.TAG_HAS_VALUE;		
			return exchangeSpecialInstructions;		
		} else {		
			return null; 		
		}		
	...
	public boolean hasExchangeSpecialInstructions() { return hasExchangeSpecialInstructions != FixUtils.TAG_HAS_NO_VALUE; } 		
		
	public void setExchangeSpecialInstructions(byte[] src) {		
		if (src == null ) return;
		if (hasExchangeSpecialInstructions()) FixUtils.fillNul(exchangeSpecialInstructions);		
		FixUtils.copy(exchangeSpecialInstructions, src); 		
		hasExchangeSpecialInstructions = FixUtils.TAG_HAS_VALUE;		
	}		
			
	public void setExchangeSpecialInstructions(String str) {		
		if (str == null ) return;
		if (hasExchangeSpecialInstructions()) FixUtils.fillNul(exchangeSpecialInstructions);		
		byte[] src = str.getBytes(); 		
		FixUtils.copy(exchangeSpecialInstructions, src); 		
	...
	public byte getOrderCategory() { 		
		if ( hasOrderCategory()) {		
			...
			orderCategory = FixUtils.getTagCharValue(buf, err);
	...
	public long getSideLiquidityInd() { 		
		if ( hasSideLiquidityInd()) {		
			if (hasSideLiquidityInd == FixUtils.TAG_HAS_VALUE) {		
				return sideLiquidityInd; 		
			} else {		
		
				buf.position(hasSideLiquidityInd);		
		
			sideLiquidityInd = FixUtils.getTagIntValue(buf, err);
			...
			hasSideLiquidityInd = FixUtils.TAG_HAS_VALUE;		
			return sideLiquidityInd;		
		} else {		
			return 0; 		
		}		
	...
	public boolean hasSideLiquidityInd() { return hasSideLiquidityInd != FixUtils.TAG_HAS_NO_VALUE; } 		
		
	public void setSideLiquidityInd(long src) {		
		sideLiquidityInd = src;
		hasSideLiquidityInd = FixUtils.TAG_HAS_VALUE;		
	}

	public void setSideLiquidityInd(byte[] src) {		
		if (src == null ) return;
		if (hasSideLiquidityInd()) sideLiquidityInd = FixUtils.TAG_HAS_NO_VALUE;		
		sideLiquidityInd = FixUtils.longValueOf(src, 0, src.length);
		hasSideLiquidityInd = FixUtils.TAG_HAS_VALUE;		
	}		
			
	public void setSideLiquidityInd(String str) {		
		if (str == null ) return;
		if (hasSideLiquidityInd()) sideLiquidityInd = FixUtils.TAG_HAS_NO_VALUE;		
		byte[] src = str.getBytes(); 		
		sideLiquidityInd = FixUtils.longValueOf(src, 0, src.length);
	...
	**/
	...
	public String toString() {
		...
		if (hasExchangeSpecialInstructions()) s += "ExchangeSpecialInstructions(1139)= " + new String( FixUtils.trim(getExchangeSpecialInstructions()) ) + "\n" ; 
		if (hasOrderCategory()) s += "OrderCategory(1115)= " + getOrderCategory() + "\n" ; 
		if (hasSideLiquidityInd()) s += "SideLiquidityInd(1444)= " + getSideLiquidityInd() + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if ((hasExchangeSpecialInstructions() && !msg.hasExchangeSpecialInstructions()) || (!hasExchangeSpecialInstructions() && msg.hasExchangeSpecialInstructions())) return false;
		if (!(!hasExchangeSpecialInstructions() && !msg.hasExchangeSpecialInstructions()) && !FixUtils.equals(getExchangeSpecialInstructions(), msg.getExchangeSpecialInstructions())) return false;
		...
		if ((hasSideLiquidityInd() && !msg.hasSideLiquidityInd()) || (!hasSideLiquidityInd() && msg.hasSideLiquidityInd())) return false;
		if (!(!hasSideLiquidityInd() && !msg.hasSideLiquidityInd()) && !(getSideLiquidityInd()==msg.getSideLiquidityInd())) return false;
		return true;
	}
	public FixTrdCapRptSideGrp clone ( FixTrdCapRptSideGrp out ) {
		if ( hasSide())
			out.setSide(getSide());
		if ( hasSideExecID())
		...
		if ( hasExchangeSpecialInstructions())
			out.setExchangeSpecialInstructions(getExchangeSpecialInstructions());
		if ( hasOrderCategory())
			out.setOrderCategory(getOrderCategory());
		if ( hasSideLiquidityInd())
			out.setSideLiquidityInd(getSideLiquidityInd());
		int count = 0;
		count = 0;
