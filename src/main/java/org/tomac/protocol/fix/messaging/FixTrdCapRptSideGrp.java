...
public class FixTrdCapRptSideGrp extends FixGroup {
	...
	long sideLiquidityInd = 0;		
		...
		FixContAmtGrp[] contAmtGrp;
		FixStipulations[] stipulations;
		FixMiscFeesGrp[] miscFeesGrp;
		FixTrdAllocGrp[] trdAllocGrp;
		FixSideTrdRegTS[] sideTrdRegTS;
		FixSettlDetails[] settlDetails;
		FixTradeReportOrderDetail tradeReportOrderDetail;
	...
	public void encode(ByteBuffer out) {
		...
		for (FixContAmtGrp fixContAmtGrp : contAmtGrp) fixContAmtGrp.encode(out);
		...
		for (FixTrdAllocGrp fixTrdAllocGrp : trdAllocGrp) fixTrdAllocGrp.encode(out);
		for (FixSideTrdRegTS fixSideTrdRegTS : sideTrdRegTS) fixSideTrdRegTS.encode(out);
		for (FixSettlDetails fixSettlDetails : settlDetails) fixSettlDetails.encode(out);
	...
	public byte[] getExchangeSpecialInstructions() { 		
		if ( hasExchangeSpecialInstructions()) {		
			...
			FixMessage.getTagStringValue(buf, exchangeSpecialInstructions, 0, exchangeSpecialInstructions.length, err);
	...
	public void setExchangeSpecialInstructions(byte[] src) {		
		if (src == null ) return;
		if (hasExchangeSpecialInstructions()) FixUtils.fillSpace(exchangeSpecialInstructions);		
	...
	public long getSideLiquidityInd() { 		
		if ( hasSideLiquidityInd()) {		
			...
			sideLiquidityInd = FixMessage.getTagIntValue(buf, err);
