...
public class FixRejectedCancelReplace extends FixInMessage {
	...
	public FixRejectedCancelReplace() {
		super(FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasClOrdID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClOrdID missing", FixTags.CLORDID_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		if (!hasOrderID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderID missing", FixTags.ORDERID_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		if (!hasOrdStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdStatus missing", FixTags.ORDSTATUS_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		if (!hasOrigClOrdID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrigClOrdID missing", FixTags.ORIGCLORDID_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		if (!hasClientID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClientID missing", FixTags.CLIENTID_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		if (!hasCxlRejResponseTo()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CxlRejResponseTo missing", FixTags.CXLREJRESPONSETO_INT, FixMessageInfo.MessageTypes.REJECTEDCANCELREPLACE);
			return false;
		}
		return true;
	}
	...
	**/
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixRejectedCancelReplace)) return false;

		FixRejectedCancelReplace msg = (FixRejectedCancelReplace) o;
	...
	public FixRejectedCancelReplace clone ( FixRejectedCancelReplace out ) {
		if ( hasClOrdID())
