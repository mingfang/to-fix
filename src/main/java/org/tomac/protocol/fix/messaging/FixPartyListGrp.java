...
public class FixPartyListGrp extends FixGroup {
        			...
        			if ( partyDetail.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !relatedPartyGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORELATEDPARTYIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	**/
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < relatedPartyGrp.length; i++)
			if (!relatedPartyGrp[i].equals(msg.relatedPartyGrp[i])) return false;
		return true;
	}
