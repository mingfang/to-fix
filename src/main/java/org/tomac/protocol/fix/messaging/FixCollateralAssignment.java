...
public class FixCollateralAssignment extends FixInMessage {
	...
	private short hasCollAsgnRefID;
	byte[] collAsgnRefID = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
	...
	public FixCollateralAssignment() {
		super(FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT);
		...
		hasCollAsgnRefID = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.COLLASGNREFID_INT:		
            		hasCollAsgnRefID = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasCollAsgnID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.COLLASGNID_INT, FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT_INT);
			return false;
		}
		if (!hasCollAsgnReason()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.COLLASGNREASON_INT, FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT_INT);
			return false;
		}
		if (!hasCollAsgnTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.COLLASGNTRANSTYPE_INT, FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT_INT);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.COLLATERALASSIGNMENT_INT);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasCollAsgnRefID()) {		
			out.put(FixTags.COLLASGNREFID);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,collAsgnRefID); 		
	...
	public byte[] getCollAsgnID() { 		
		if ( hasCollAsgnID()) {		
			...
			FixUtils.getTagStringValue(buf, collAsgnID, 0, collAsgnID.length, err);
	...
	public void setCollAsgnID(byte[] src) {		
		if (src == null ) return;
		if (hasCollAsgnID()) FixUtils.fillNul(collAsgnID);		
	...
	public long getCollAsgnTransType() { 		
		if ( hasCollAsgnTransType()) {		
			...
			collAsgnTransType = FixUtils.getTagIntValue(buf, err);
	...
	public void crackCollAsgnRefID() {		
		getCollAsgnRefID();		
	}		
			
	public byte[] getCollAsgnRefID() { 		
		if ( hasCollAsgnRefID()) {		
			if (hasCollAsgnRefID == FixUtils.TAG_HAS_VALUE) {		
				return collAsgnRefID; 		
			} else {

				buf.position(hasCollAsgnRefID);

			FixUtils.getTagStringValue(buf, collAsgnRefID, 0, collAsgnRefID.length, err);
			...
			hasCollAsgnRefID = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return collAsgnRefID;		
		} else {		
			return null; 		
	...
	public boolean hasCollAsgnRefID() { return hasCollAsgnRefID != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setCollAsgnRefID(byte[] src) {		
		if (src == null ) return;
		if (hasCollAsgnRefID()) FixUtils.fillNul(collAsgnRefID);		
		FixUtils.copy(collAsgnRefID, src); 		
		hasCollAsgnRefID = FixUtils.TAG_HAS_VALUE;		
	}

	public void setCollAsgnRefID(String str) {		
		if (str == null ) return;
		if (hasCollAsgnRefID()) FixUtils.fillNul(collAsgnRefID);		
		byte[] src = str.getBytes(); 		
		FixUtils.copy(collAsgnRefID, src); 		
	...
	**/
	...
	public String toString() {
		...
		if (hasCollAsgnRefID()) s += "CollAsgnRefID(907)= " + new String( FixUtils.trim(getCollAsgnRefID()) ) + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if ((hasCollAsgnRefID() && !msg.hasCollAsgnRefID()) || (!hasCollAsgnRefID() && msg.hasCollAsgnRefID())) return false;
		if (!(!hasCollAsgnRefID() && !msg.hasCollAsgnRefID()) && !FixUtils.equals(getCollAsgnRefID(), msg.getCollAsgnRefID())) return false;
	...
	@Override
	public FixCollateralAssignment clone () {
		FixCollateralAssignment out = new FixCollateralAssignment();
	...
	public FixCollateralAssignment clone ( FixCollateralAssignment out ) {
		if ( hasCollAsgnID())
		...
		if ( hasCollAsgnRefID())
			out.setCollAsgnRefID(getCollAsgnRefID());
		if ( hasTransactTime())
