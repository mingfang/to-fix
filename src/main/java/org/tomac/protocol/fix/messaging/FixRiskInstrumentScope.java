        			...
        			if ( tag == FixTags.NORISKSECURITYALTID_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !riskSecAltIDGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORISKSECURITYALTID_INT);
        						return repeatingGroupTag;
        					}
        					count++;
