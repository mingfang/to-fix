...
public class FixPtysSubGrp extends FixGroup {
	...
	public FixPtysSubGrp(boolean isRequired) {
	...
	public byte[] getPartySubID() { 		
		if ( hasPartySubID()) {		
			...
			FixUtils.getTagStringValue(buf, partySubID, 0, partySubID.length, err);
	...
	public void setPartySubID(byte[] src) {		
		if (src == null ) return;
		if (hasPartySubID()) FixUtils.fillNul(partySubID);		
	...
	public long getPartySubIDType() { 		
		if ( hasPartySubIDType()) {		
			...
			partySubIDType = FixUtils.getTagIntValue(buf, err);
