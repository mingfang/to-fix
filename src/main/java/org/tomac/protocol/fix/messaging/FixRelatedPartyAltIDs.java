        			...
        			if ( tag == FixTags.NORELATEDPARTYALTSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !relatedAltPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORELATEDPARTYALTSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
