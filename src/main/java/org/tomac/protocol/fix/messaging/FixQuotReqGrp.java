...
public class FixQuotReqGrp extends FixGroup {
	...
	public FixQuotReqGrp() {
		this(false);
	}

	public FixQuotReqGrp(boolean isRequired) {
	...
	public void encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(quotReqLegsGrp)>0) {
			out.put(FixTags.NOLEGS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(quotReqLegsGrp));

			out.put(FixUtils.SOH);

		}
		for (FixQuotReqLegsGrp fixQuotReqLegsGrp : quotReqLegsGrp) if (fixQuotReqLegsGrp.hasGroup()) fixQuotReqLegsGrp.encode(out);
	...
	public long getQuotePriceType() { 		
		if ( hasQuotePriceType()) {		
			...
			quotePriceType = FixUtils.getTagIntValue(buf, err);
	...
	**/
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixQuotReqGrp)) return false;

		FixQuotReqGrp msg = (FixQuotReqGrp) o;
	...
	public FixQuotReqGrp clone ( FixQuotReqGrp out ) {
		if ( hasPrevClosePx())
