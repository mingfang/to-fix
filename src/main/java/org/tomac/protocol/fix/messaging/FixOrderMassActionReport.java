...
public class FixOrderMassActionReport extends FixInMessage {
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.ORDERMASSACTIONREPORT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasMassActionReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.MASSACTIONREPORTID_INT, FixMessageInfo.MessageTypes.ORDERMASSACTIONREPORT_INT);
			return false;
		}
		if (!hasMassActionType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.MASSACTIONTYPE_INT, FixMessageInfo.MessageTypes.ORDERMASSACTIONREPORT_INT);
			return false;
		}
		if (!hasMassActionScope()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.MASSACTIONSCOPE_INT, FixMessageInfo.MessageTypes.ORDERMASSACTIONREPORT_INT);
			return false;
		}
		if (!hasMassActionResponse()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.MASSACTIONRESPONSE_INT, FixMessageInfo.MessageTypes.ORDERMASSACTIONREPORT_INT);
			return false;
		}
		return true;
	}
	...
	public long getMassActionResponse() { 		
		if ( hasMassActionResponse()) {		
			...
			massActionResponse = FixUtils.getTagIntValue(buf, err);
	...
	public long getMassActionRejectReason() { 		
		if ( hasMassActionRejectReason()) {		
			...
			massActionRejectReason = FixUtils.getTagIntValue(buf, err);
