        			...
        			if ( tag == FixTags.NOROOTPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !rootSubParties[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOROOTPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
