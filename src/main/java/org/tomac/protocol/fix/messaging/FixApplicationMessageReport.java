...
public class FixApplicationMessageReport extends FixInMessage {
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREPORT_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasApplReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.APPLREPORTID_INT, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREPORT_INT);
			return false;
		}
		if (!hasApplReportType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.APPLREPORTTYPE_INT, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREPORT_INT);
			return false;
		}
		return true;
	}
	...
	public byte[] getApplReportID() { 		
		if ( hasApplReportID()) {		
			...
			FixUtils.getTagStringValue(buf, applReportID, 0, applReportID.length, err);
	...
	public long getApplReportType() { 		
		if ( hasApplReportType()) {		
			...
			applReportType = FixUtils.getTagIntValue(buf, err);
