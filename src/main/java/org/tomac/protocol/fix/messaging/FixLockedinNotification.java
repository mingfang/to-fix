...
public class FixLockedinNotification extends FixInMessage {
	...
	public FixLockedinNotification() {
		...
		hasLockedIn = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.LOCKEDIN_INT:		
            		hasLockedIn = (short) buf.position();		
            		FixMessage.getNext(buf, err);		
                	break;
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasAvgPx()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag AvgPx missing", FixTags.AVGPX_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasCumQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CumQty missing", FixTags.CUMQTY_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasCurrency()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Currency missing", FixTags.CURRENCY_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasExecID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecID missing", FixTags.EXECID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasExecTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecTransType missing", FixTags.EXECTRANSTYPE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasOrderID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderID missing", FixTags.ORDERID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasOrdStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdStatus missing", FixTags.ORDSTATUS_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasSecurityID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityID missing", FixTags.SECURITYID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Side missing", FixTags.SIDE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasSymbol()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Symbol missing", FixTags.SYMBOL_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasText()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Text missing", FixTags.TEXT_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransactTime missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasSecurityDesc()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityDesc missing", FixTags.SECURITYDESC_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasClientID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClientID missing", FixTags.CLIENTID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasExecType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecType missing", FixTags.EXECTYPE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasLeavesQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LeavesQty missing", FixTags.LEAVESQTY_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasContraBroker()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ContraBroker missing", FixTags.CONTRABROKER_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasPriceType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag PriceType missing", FixTags.PRICETYPE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTransBkdTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransBkdTime missing", FixTags.TRANSBKDTIME_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTradeReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportID missing", FixTags.TRADEREPORTID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasClearingInstruction()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClearingInstruction missing", FixTags.CLEARINGINSTRUCTION_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTradeType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeType missing", FixTags.TRADETYPE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasPublishTrdIndicator()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag PublishTrdIndicator missing", FixTags.PUBLISHTRDINDICATOR_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTradeReportType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportType missing", FixTags.TRADEREPORTTYPE_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasTradeID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeID missing", FixTags.TRADEID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		if (!hasSubMktID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SubMktID missing", FixTags.SUBMKTID_INT, FixMessageInfo.MessageTypes.LOCKEDINNOTIFICATION);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasLockedIn()) {		
			out.put(FixTags.LOCKEDIN);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,lockedIn); 		
	...
	public void crackLockedIn() {		
		getLockedIn();		
	}		
			
	public byte getLockedIn() { 		
		if ( hasLockedIn()) {		
			if (hasLockedIn == FixUtils.TAG_HAS_VALUE) {		
				return lockedIn; 		
			} else {

				buf.position(hasLockedIn);

			lockedIn = FixMessage.getTagCharValue(buf, err);
			if( !err.hasError() && (lockedIn != (byte)'M') && true)
				err.setError((int)FixMessageInfo.SessionRejectReason.VALUE_IS_INCORRECT_OUT_OF_RANGE_FOR_THIS_TAG,
					"Tag msgType missing got " + 9857);		
				if (err.hasError()) {		
					buf.position(0);		
					return (byte)'0';		
				}		
			}		
			hasLockedIn = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return lockedIn;		
		} else {		
			return (byte)'0'; 		
	...
	public boolean hasLockedIn() { return hasLockedIn != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setLockedIn(byte[] src) {		
		if (src == null ) return;
		if (hasLockedIn()) lockedIn = (byte)' ';		
		lockedIn = src[0];		
		hasLockedIn = FixUtils.TAG_HAS_VALUE;		
	}
	...
	public void setLockedIn(String str) {		
		if (str == null ) return;
		if (hasLockedIn()) lockedIn = (byte)' ';		
		byte[] src = str.getBytes(); 		
		lockedIn = src[0];		
	...
	**/
	...
	public String toString() {
		...
		if (hasLockedIn()) s += "LockedIn(9857)= " + getLockedIn() + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if ((hasLockedIn() && !msg.hasLockedIn()) || (!hasLockedIn() && msg.hasLockedIn())) return false;
		if (!(!hasLockedIn() && !msg.hasLockedIn()) && !(getLockedIn()==msg.getLockedIn())) return false;
	...
	public FixLockedinNotification clone ( FixLockedinNotification out ) {
		if ( hasAvgPx())
