...
public class FixTradingSessionStatusRequest extends FixInMessage {
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasTradSesReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradSesReqID missing", FixTags.TRADSESREQID_INT, FixMessageInfo.MessageTypes.TRADINGSESSIONSTATUSREQUEST);
			return false;
		}
		if (!hasSubscriptionRequestType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SubscriptionRequestType missing", FixTags.SUBSCRIPTIONREQUESTTYPE_INT, FixMessageInfo.MessageTypes.TRADINGSESSIONSTATUSREQUEST);
			return false;
		}
		return true;
	}
