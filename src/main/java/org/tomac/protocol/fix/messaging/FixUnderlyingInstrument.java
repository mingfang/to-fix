        			...
        			if ( tag == FixTags.NOUNDERLYINGSECURITYALTID_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !undSecAltIDGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDERLYINGSECURITYALTID_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !underlyingStipulations[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDERLYINGSTIPS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !undlyInstrumentParties[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDLYINSTRUMENTPARTIES_INT);
        						return repeatingGroupTag;
        					}
        					count++;
