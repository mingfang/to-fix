        			...
        			if ( tag == FixTags.NORELATIONSHIPRISKINSTRUMENTS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !relationshipRiskInstrumentScope[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORELATIONSHIPRISKINSTRUMENTS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !relationshipRiskWarningLevels[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORELATIONSHIPRISKWARNINGLEVELS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
