...
public class FixApplicationMessageRequest extends FixInMessage {
	...
	public FixApplicationMessageRequest() {
		super(FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREQUEST);
		...
		applIDRequestGrp = new FixApplIDRequestGrp[FixUtils.FIX_MAX_NOINGROUP];
		for (int i= 0; i<FixUtils.FIX_MAX_NOINGROUP; i++) applIDRequestGrp[i] = new FixApplIDRequestGrp();
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !applIDRequestGrp[count].isKeyTag(repeatingGroupTag) ) {
        					...
        					count++;
        					repeatingGroupTag = applIDRequestGrp[count].setBuffer( repeatingGroupTag, buf, err);	
        					if (err.hasError()) break; 		
        				}
        				if (err.hasError()) break;
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREQUEST_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasApplReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.APPLREQID_INT, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREQUEST_INT);
			return false;
		}
		if (!hasApplReqType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.APPLREQTYPE_INT, FixMessageInfo.MessageTypes.APPLICATIONMESSAGEREQUEST_INT);
			return false;
		}
		return true;
	}
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(applIDRequestGrp)>0) {
			out.put(FixTags.NOAPPLIDS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(applIDRequestGrp));

			out.put(FixUtils.SOH);

		}
		for (FixApplIDRequestGrp fixApplIDRequestGrp : applIDRequestGrp) if (fixApplIDRequestGrp.hasGroup()) fixApplIDRequestGrp.encode(out);
	...
	**/
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixApplicationMessageRequest)) return false;

		FixApplicationMessageRequest msg = (FixApplicationMessageRequest) o;
	...
	@Override
	public FixApplicationMessageRequest clone () {
		FixApplicationMessageRequest out = new FixApplicationMessageRequest();
	...
	public FixApplicationMessageRequest clone ( FixApplicationMessageRequest out ) {
		if ( hasApplReqID())
		...
		int count = 0;
		count = 0;
		for (FixApplIDRequestGrp fixApplIDRequestGrp : applIDRequestGrp) {
			if (!fixApplIDRequestGrp.hasGroup()) continue;
			out.applIDRequestGrp[count] = fixApplIDRequestGrp.clone( out.applIDRequestGrp[count] );
			count++;
		}
		count = 0;
