...
public class FixSecurityTypes extends FixInMessage {
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasSecurityReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityReqID missing", FixTags.SECURITYREQID_INT, FixMessageInfo.MessageTypes.SECURITYTYPES);
			return false;
		}
		if (!hasSecurityResponseID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityResponseID missing", FixTags.SECURITYRESPONSEID_INT, FixMessageInfo.MessageTypes.SECURITYTYPES);
			return false;
		}
		if (!hasSecurityResponseType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityResponseType missing", FixTags.SECURITYRESPONSETYPE_INT, FixMessageInfo.MessageTypes.SECURITYTYPES);
			return false;
		}
		return true;
	}
