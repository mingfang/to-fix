...
public class FixDerivativeSecurityList extends FixInMessage {
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.DERIVATIVESECURITYLIST_INT);
                			break;
                		}
					}
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(relSymDerivSecGrp)>0) {
			out.put(FixTags.NORELATEDSYM);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(relSymDerivSecGrp));

			out.put(FixUtils.SOH);

		}
		for (FixRelSymDerivSecGrp fixRelSymDerivSecGrp : relSymDerivSecGrp) if (fixRelSymDerivSecGrp.hasGroup()) fixRelSymDerivSecGrp.encode(out);
