...
public class FixNewOrderList extends FixInMessage {
	...
	public int encode(ByteBuffer out) {
		...
		if (FixUtils.getNoInGroup(listOrdGrp)>0) {
			out.put(FixTags.NOORDERS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(listOrdGrp));

			out.put(FixUtils.SOH);

		}
		for (FixListOrdGrp fixListOrdGrp : listOrdGrp) if (fixListOrdGrp.hasGroup()) fixListOrdGrp.encode(out);
