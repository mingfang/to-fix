...
public class FixQuote extends FixInMessage {
	...
	private short hasBidSwapPoints;
	long bidSwapPoints = 0;		
	private short hasOfferSwapPoints;
	long offerSwapPoints = 0;		
	...
	public FixQuote() {
		...
		hasBidSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
		hasOfferSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
        ...
        while ( buf.hasRemaining() ) {
            ...
            switch (tag) {		
            	...
            	case FixTags.OFFERFORWARDPOINTS_INT:		
            		...
            		FixUtils.getNext(buf, err);		
                	break;
            	case FixTags.BIDSWAPPOINTS_INT:		
            		hasBidSwapPoints = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
            	case FixTags.OFFERSWAPPOINTS_INT:		
            		hasOfferSwapPoints = (short) buf.position();		
            		FixUtils.getNext(buf, err);		
                	break;
        			...
        			if ( standardHeader.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !quotQualGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOQUOTEQUALIFIERS_INT);
        						return;
        					}
        					count++;
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.QUOTE_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasQuoteID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.QUOTEID_INT, FixMessageInfo.MessageTypes.QUOTE_INT);
			return false;
		}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasBidSwapPoints()) {		
			out.put(FixTags.BIDSWAPPOINTS);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out, (long)bidSwapPoints);
		
			out.put(FixUtils.SOH);		
		}		
		if (hasOfferSwapPoints()) {		
			out.put(FixTags.OFFERSWAPPOINTS);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out, (long)offerSwapPoints);
		...
		if (FixUtils.getNoInGroup(legQuotGrp)>0) {
			out.put(FixTags.NOLEGS);

			out.put((byte) '=' );

			FixUtils.put(out, FixUtils.getNoInGroup(legQuotGrp));

			out.put(FixUtils.SOH);

		}
		for (FixLegQuotGrp fixLegQuotGrp : legQuotGrp) if (fixLegQuotGrp.hasGroup()) fixLegQuotGrp.encode(out);
	...
	public long getBidPx() { 		
		if ( hasBidPx()) {		
			...
			bidPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferPx() { 		
		if ( hasOfferPx()) {		
			...
			offerPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getMktBidPx() { 		
		if ( hasMktBidPx()) {		
			...
			mktBidPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getMktOfferPx() { 		
		if ( hasMktOfferPx()) {		
			...
			mktOfferPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getMinBidSize() { 		
		if ( hasMinBidSize()) {		
			...
			minBidSize = FixUtils.getTagFloatValue(buf, err);
	...
	public long getBidSize() { 		
		if ( hasBidSize()) {		
			...
			bidSize = FixUtils.getTagFloatValue(buf, err);
	...
	public long getMinOfferSize() { 		
		if ( hasMinOfferSize()) {		
			...
			minOfferSize = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferSize() { 		
		if ( hasOfferSize()) {		
			...
			offerSize = FixUtils.getTagFloatValue(buf, err);
	...
	public long getBidSpotRate() { 		
		if ( hasBidSpotRate()) {		
			...
			bidSpotRate = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferSpotRate() { 		
		if ( hasOfferSpotRate()) {		
			...
			offerSpotRate = FixUtils.getTagFloatValue(buf, err);
	...
	public long getBidForwardPoints() { 		
		if ( hasBidForwardPoints()) {		
			...
			bidForwardPoints = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferForwardPoints() { 		
		if ( hasOfferForwardPoints()) {		
			...
			offerForwardPoints = FixUtils.getTagFloatValue(buf, err);
	...
	public void crackBidSwapPoints() {		
		getBidSwapPoints();		
	}		
			
	public long getBidSwapPoints() { 		
		if ( hasBidSwapPoints()) {		
			if (hasBidSwapPoints == FixUtils.TAG_HAS_VALUE) {		
				return bidSwapPoints; 		
			} else {

				buf.position(hasBidSwapPoints);

			bidSwapPoints = FixUtils.getTagFloatValue(buf, err);
			...
			hasBidSwapPoints = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return bidSwapPoints;		
		} else {		
			return 0; 		
	...
	public boolean hasBidSwapPoints() { return hasBidSwapPoints != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setBidSwapPoints(byte[] src) {		
		if (src == null ) return;
		if (hasBidSwapPoints()) bidSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
		bidSwapPoints = FixUtils.longValueOf(src, 0, src.length);
		hasBidSwapPoints = FixUtils.TAG_HAS_VALUE;		
	}

	public void setBidSwapPoints(long src) {		
		bidSwapPoints = src;
		hasBidSwapPoints = FixUtils.TAG_HAS_VALUE;		
	}

	public void setBidSwapPoints(String str) {		
		if (str == null ) return;
		if (hasBidSwapPoints()) bidSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
		byte[] src = str.getBytes(); 		
		bidSwapPoints = FixUtils.longValueOf(src, 0, src.length);
	...
	public void crackOfferSwapPoints() {		
		getOfferSwapPoints();		
	}		
			
	public long getOfferSwapPoints() { 		
		if ( hasOfferSwapPoints()) {		
			if (hasOfferSwapPoints == FixUtils.TAG_HAS_VALUE) {		
				return offerSwapPoints; 		
			} else {

				buf.position(hasOfferSwapPoints);

			offerSwapPoints = FixUtils.getTagFloatValue(buf, err);
			...
			hasOfferSwapPoints = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return offerSwapPoints;		
		} else {		
			return 0; 		
	...
	public boolean hasOfferSwapPoints() { return hasOfferSwapPoints != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setOfferSwapPoints(byte[] src) {		
		if (src == null ) return;
		if (hasOfferSwapPoints()) offerSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
		offerSwapPoints = FixUtils.longValueOf(src, 0, src.length);
		hasOfferSwapPoints = FixUtils.TAG_HAS_VALUE;		
	}

	public void setOfferSwapPoints(long src) {		
		offerSwapPoints = src;
		hasOfferSwapPoints = FixUtils.TAG_HAS_VALUE;		
	}

	public void setOfferSwapPoints(String str) {		
		if (str == null ) return;
		if (hasOfferSwapPoints()) offerSwapPoints = FixUtils.TAG_HAS_NO_VALUE;		
		byte[] src = str.getBytes(); 		
		offerSwapPoints = FixUtils.longValueOf(src, 0, src.length);
	...
	public long getMidPx() { 		
		if ( hasMidPx()) {		
			...
			midPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getBidYield() { 		
		if ( hasBidYield()) {		
			...
			bidYield = FixUtils.getTagFloatValue(buf, err);
	...
	public long getMidYield() { 		
		if ( hasMidYield()) {		
			...
			midYield = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferYield() { 		
		if ( hasOfferYield()) {		
			...
			offerYield = FixUtils.getTagFloatValue(buf, err);
	...
	public long getBidForwardPoints2() { 		
		if ( hasBidForwardPoints2()) {		
			...
			bidForwardPoints2 = FixUtils.getTagFloatValue(buf, err);
	...
	public long getOfferForwardPoints2() { 		
		if ( hasOfferForwardPoints2()) {		
			...
			offerForwardPoints2 = FixUtils.getTagFloatValue(buf, err);
	...
	public long getSettlCurrBidFxRate() { 		
		if ( hasSettlCurrBidFxRate()) {		
			...
			settlCurrBidFxRate = FixUtils.getTagFloatValue(buf, err);
	...
	public long getSettlCurrOfferFxRate() { 		
		if ( hasSettlCurrOfferFxRate()) {		
			...
			settlCurrOfferFxRate = FixUtils.getTagFloatValue(buf, err);
	...
	**/
	...
	public String toString() {
		...
		if (hasBidSwapPoints()) s += "BidSwapPoints(1065)= " + getBidSwapPoints() / FixUtils.FIX_FLOAT_NUMBER_OF_DECIMALS + "\n" ; 
		if (hasOfferSwapPoints()) s += "OfferSwapPoints(1066)= " + getOfferSwapPoints() / FixUtils.FIX_FLOAT_NUMBER_OF_DECIMALS + "\n" ; 
	...
	public boolean equals(Object o) {
		if (! ( o instanceof FixQuote)) return false;

		FixQuote msg = (FixQuote) o;
		...
		if ((hasBidSwapPoints() && !msg.hasBidSwapPoints()) || (!hasBidSwapPoints() && msg.hasBidSwapPoints())) return false;
		if (!(!hasBidSwapPoints() && !msg.hasBidSwapPoints()) && !(getBidSwapPoints()==msg.getBidSwapPoints())) return false;
		if ((hasOfferSwapPoints() && !msg.hasOfferSwapPoints()) || (!hasOfferSwapPoints() && msg.hasOfferSwapPoints())) return false;
		if (!(!hasOfferSwapPoints() && !msg.hasOfferSwapPoints()) && !(getOfferSwapPoints()==msg.getOfferSwapPoints())) return false;
	...
	@Override
	public FixQuote clone () {
		FixQuote out = new FixQuote();
	...
	public FixQuote clone ( FixQuote out ) {
		if ( hasQuoteReqID())
		...
		if ( hasBidSwapPoints())
			out.setBidSwapPoints(getBidSwapPoints());
		if ( hasOfferSwapPoints())
			out.setOfferSwapPoints(getOfferSwapPoints());
		if ( hasMidPx())
