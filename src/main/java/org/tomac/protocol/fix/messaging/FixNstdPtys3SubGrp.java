...
public class FixNstdPtys3SubGrp extends FixGroup {
	...
	public byte[] getNested3PartySubID() { 		
		if ( hasNested3PartySubID()) {		
			...
			FixMessage.getTagStringValue(buf, nested3PartySubID, 0, nested3PartySubID.length, err);
	...
	public void setNested3PartySubID(byte[] src) {		
		if (src == null ) return;
		if (hasNested3PartySubID()) FixUtils.fillSpace(nested3PartySubID);		
	...
	public long getNested3PartySubIDType() { 		
		if ( hasNested3PartySubIDType()) {		
			...
			nested3PartySubIDType = FixMessage.getTagIntValue(buf, err);
