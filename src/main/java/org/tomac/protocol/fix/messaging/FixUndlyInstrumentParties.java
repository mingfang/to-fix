        			...
        			if ( tag == FixTags.NOUNDLYINSTRUMENTPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !undlyInstrumentPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOUNDLYINSTRUMENTPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
