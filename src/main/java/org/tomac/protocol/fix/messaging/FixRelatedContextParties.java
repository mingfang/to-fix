...
public class FixRelatedContextParties extends FixGroup {
        			...
        			if ( tag == FixTags.NORELATEDCONTEXTPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !relatedContextPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NORELATEDCONTEXTPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public void setRelatedContextPartyID(byte[] src) {		
		if (src == null ) return;
		if (hasRelatedContextPartyID()) FixUtils.fillNul(relatedContextPartyID);		
