        			...
        			if ( tag == FixTags.NOHOPS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !hopGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOHOPS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
