...
public class FixTrdInstrmtLegGrp extends FixGroup {
        			...
        			if ( instrumentLeg.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !tradeCapLegUnderlyingsGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOOFLEGUNDERLYINGS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getLegReportID() { 		
		if ( hasLegReportID()) {		
			...
			FixUtils.getTagStringValue(buf, legReportID, 0, legReportID.length, err);
	...
	public void setLegReportID(byte[] src) {		
		if (src == null ) return;
		if (hasLegReportID()) FixUtils.fillNul(legReportID);		
	...
	public long getLegNumber() { 		
		if ( hasLegNumber()) {		
			...
			legNumber = FixUtils.getTagIntValue(buf, err);
