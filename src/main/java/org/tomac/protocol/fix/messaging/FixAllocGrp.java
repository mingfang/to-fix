...
public class FixAllocGrp extends FixGroup {
	...
	public FixAllocGrp(boolean isRequired) {
	...
	public long getAllocMethod() { 		
		if ( hasAllocMethod()) {		
			...
			allocMethod = FixUtils.getTagIntValue(buf, err);
	...
	public boolean getNotifyBrokerOfCredit() { 		
		if ( hasNotifyBrokerOfCredit()) {		
			...
			notifyBrokerOfCredit = FixUtils.getTagCharValue(buf, err)=='Y'?true:false;
	...
	public long getAllocHandlInst() { 		
		if ( hasAllocHandlInst()) {		
			...
			allocHandlInst = FixUtils.getTagIntValue(buf, err);
	...
	public long getAllocAvgPx() { 		
		if ( hasAllocAvgPx()) {		
			...
			allocAvgPx = FixUtils.getTagFloatValue(buf, err);
	...
	public long getAllocNetMoney() { 		
		if ( hasAllocNetMoney()) {		
			...
			allocNetMoney = FixUtils.getTagFloatValue(buf, err);
	...
	public long getAllocSettlCurrAmt() { 		
		if ( hasAllocSettlCurrAmt()) {		
			...
			allocSettlCurrAmt = FixUtils.getTagFloatValue(buf, err);
	...
	public long getAllocAccruedInterestAmt() { 		
		if ( hasAllocAccruedInterestAmt()) {		
			...
			allocAccruedInterestAmt = FixUtils.getTagFloatValue(buf, err);
	...
	public long getAllocInterestAtMaturity() { 		
		if ( hasAllocInterestAtMaturity()) {		
			...
			allocInterestAtMaturity = FixUtils.getTagFloatValue(buf, err);
	...
	public long getAllocSettlInstType() { 		
		if ( hasAllocSettlInstType()) {		
			...
			allocSettlInstType = FixUtils.getTagIntValue(buf, err);
