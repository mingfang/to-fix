...
public class FixTradeReportEntry extends FixInMessage {
	...
	byte[] partyRole = new byte[FixUtils.FIX_MAX_STRING_LENGTH];		
	...
	public FixTradeReportEntry() {
		super(FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasAvgPx()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag AvgPx missing", FixTags.AVGPX_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasCumQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag CumQty missing", FixTags.CUMQTY_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasExecID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecID missing", FixTags.EXECID_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasExecTransType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecTransType missing", FixTags.EXECTRANSTYPE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasOrderID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrderID missing", FixTags.ORDERID_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasOrdStatus()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag OrdStatus missing", FixTags.ORDSTATUS_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasSide()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Side missing", FixTags.SIDE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasSymbol()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag Symbol missing", FixTags.SYMBOL_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransactTime missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasProcessCode()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ProcessCode missing", FixTags.PROCESSCODE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasSecurityDesc()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityDesc missing", FixTags.SECURITYDESC_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasExecType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ExecType missing", FixTags.EXECTYPE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasLeavesQty()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag LeavesQty missing", FixTags.LEAVESQTY_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasPartyRole()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag PartyRole missing", FixTags.PARTYROLE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasTransBkdTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransBkdTime missing", FixTags.TRANSBKDTIME_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasTradeReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportID missing", FixTags.TRADEREPORTID_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasClearingInstruction()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag ClearingInstruction missing", FixTags.CLEARINGINSTRUCTION_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasTradeType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeType missing", FixTags.TRADETYPE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		if (!hasTradeReportType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TradeReportType missing", FixTags.TRADEREPORTTYPE_INT, FixMessageInfo.MessageTypes.TRADEREPORTENTRY);
			return false;
		}
		return true;
	}
	...
	public void crackPartyRole() {		
		getPartyRole();		
	}		
			
	public byte[] getPartyRole() { 		
		if ( hasPartyRole()) {		
			...
			FixMessage.getTagStringValue(buf, partyRole, 0, partyRole.length, err);
	...
	public void setPartyRole(byte[] src) {		
		if (src == null ) return;
		if (hasPartyRole()) FixUtils.fillSpace(partyRole);		
		FixUtils.copy(partyRole, src); 		
		hasPartyRole = FixUtils.TAG_HAS_VALUE;		
	}
	...
	**/
	...
	public String toString() {
		...
		if (hasPartyRole()) s += "PartyRole(452)= " + new String( FixUtils.trim(getPartyRole()) ) + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if (!(!hasPartyRole() && !msg.hasPartyRole()) && !FixUtils.equals(getPartyRole(), msg.getPartyRole())) return false;
	...
	public FixTradeReportEntry clone ( FixTradeReportEntry out ) {
		if ( hasAvgPx())
