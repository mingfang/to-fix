...
public class FixSecurityTypeRequest extends FixInMessage {
	...
	private boolean hasRequiredTags(FixValidationError err) {
		if (!hasSecurityReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SecurityReqID missing", FixTags.SECURITYREQID_INT, FixMessageInfo.MessageTypes.SECURITYTYPEREQUEST);
			return false;
		}
		return true;
	}
