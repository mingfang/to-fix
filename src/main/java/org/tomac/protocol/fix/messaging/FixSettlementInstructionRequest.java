...
public class FixSettlementInstructionRequest extends FixInMessage {
	...
	public boolean hasRequiredTags(FixValidationError err) {
		...
		if (!hasSettlInstReqID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag SettlInstReqID missing", FixTags.SETTLINSTREQID_INT, FixMessageInfo.MessageTypes.SETTLEMENTINSTRUCTIONREQUEST);
			return false;
		}
		if (!hasTransactTime()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag TransactTime missing", FixTags.TRANSACTTIME_INT, FixMessageInfo.MessageTypes.SETTLEMENTINSTRUCTIONREQUEST);
			return false;
		}
