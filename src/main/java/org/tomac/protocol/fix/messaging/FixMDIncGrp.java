...
public class FixMDIncGrp extends FixGroup {
        			...
        			if ( instrument.isKeyTag(tag)) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !secSizesGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOOFSECSIZES_INT);
        						return repeatingGroupTag;
        					}
        					count++;
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !statsIndGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOSTATSINDICATORS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte getMDUpdateAction() { 		
		if ( hasMDUpdateAction()) {		
			...
			mDUpdateAction = FixUtils.getTagCharValue(buf, err);
	...
	public byte getDeleteReason() { 		
		if ( hasDeleteReason()) {		
			...
			deleteReason = FixUtils.getTagCharValue(buf, err);
	...
	public byte[] getMDEntryRefID() { 		
		if ( hasMDEntryRefID()) {		
			...
			FixUtils.getTagStringValue(buf, mDEntryRefID, 0, mDEntryRefID.length, err);
	...
	**/
	...
	public boolean equals(Object o) {
		...
		for (int i = 0; i < statsIndGrp.length; i++)
			if (!statsIndGrp[i].equals(msg.statsIndGrp[i])) return false;
