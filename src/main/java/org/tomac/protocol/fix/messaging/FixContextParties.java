...
public class FixContextParties extends FixGroup {
        			...
        			if ( tag == FixTags.NOCONTEXTPARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !contextPtysSubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NOCONTEXTPARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getContextPartyID() { 		
		if ( hasContextPartyID()) {		
			...
			FixUtils.getTagStringValue(buf, contextPartyID, 0, contextPartyID.length, err);
	...
	public byte getContextPartyIDSource() { 		
		if ( hasContextPartyIDSource()) {		
			...
			contextPartyIDSource = FixUtils.getTagCharValue(buf, err);
	...
	public long getContextPartyRole() { 		
		if ( hasContextPartyRole()) {		
			...
			contextPartyRole = FixUtils.getTagIntValue(buf, err);
