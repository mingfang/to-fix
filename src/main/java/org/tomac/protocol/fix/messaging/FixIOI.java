...
public class FixIOI extends FixInMessage {
	...
	FixInstrmtLegIOIGrp[] instrmtLegIOIGrp;
	FixIOIQualGrp[] iOIQualGrp;
	FixRoutingGrp[] routingGrp;
	...
	public byte getIOITransType() { 		
		if ( hasIOITransType()) {		
			...
			iOITransType = FixMessage.getTagCharValue(buf, err);
	...
	public byte[] getIOIRefID() { 		
		if ( hasIOIRefID()) {		
			...
			FixMessage.getTagStringValue(buf, iOIRefID, 0, iOIRefID.length, err);
	...
	public byte[] getIOIQty() { 		
		if ( hasIOIQty()) {		
			...
			FixMessage.getTagStringValue(buf, iOIQty, 0, iOIQty.length, err);
	...
	public byte getIOIQltyInd() { 		
		if ( hasIOIQltyInd()) {		
			...
			iOIQltyInd = FixMessage.getTagCharValue(buf, err);
	...
	public boolean getIOINaturalFlag() { 		
		if ( hasIOINaturalFlag()) {		
			...
			iOINaturalFlag = FixMessage.getTagCharValue(buf, err)=='Y'?true:false;
