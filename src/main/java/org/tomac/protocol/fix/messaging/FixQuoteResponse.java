...
public class FixQuoteResponse extends FixInMessage {
            	...
            	default:
        			if ( standardHeader.isKeyTag(tag)) {
                		...
                		if (err.hasError()) break; 		
                			...
                			err.setError((int)FixMessageInfo.SessionRejectReason.TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE, "Tag not defined for this message type", tag, FixMessageInfo.MessageTypes.QUOTERESPONSE_INT);
                			break;
                		}
					}
	...
	public boolean hasRequiredTags(FixValidationError err) {
		if (!hasQuoteRespID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.QUOTERESPID_INT, FixMessageInfo.MessageTypes.QUOTERESPONSE_INT);
			return false;
		}
		if (!hasQuoteRespType()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "Required tag missing", FixTags.QUOTERESPTYPE_INT, FixMessageInfo.MessageTypes.QUOTERESPONSE_INT);
			return false;
		}
	...
	public long getQuoteRespType() { 		
		if ( hasQuoteRespType()) {		
			...
			quoteRespType = FixUtils.getTagIntValue(buf, err);
