...
public class FixNstdPtys2SubGrp extends FixGroup {
	...
	public FixNstdPtys2SubGrp(boolean isRequired) {
	...
	public byte[] getNested2PartySubID() { 		
		if ( hasNested2PartySubID()) {		
			...
			FixUtils.getTagStringValue(buf, nested2PartySubID, 0, nested2PartySubID.length, err);
	...
	public void setNested2PartySubID(byte[] src) {		
		if (src == null ) return;
		if (hasNested2PartySubID()) FixUtils.fillNul(nested2PartySubID);		
	...
	public long getNested2PartySubIDType() { 		
		if ( hasNested2PartySubIDType()) {		
			...
			nested2PartySubIDType = FixUtils.getTagIntValue(buf, err);
