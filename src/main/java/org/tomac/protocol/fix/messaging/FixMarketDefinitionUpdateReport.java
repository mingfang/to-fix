...
public class FixMarketDefinitionUpdateReport extends FixInMessage {
	...
	private short hasMarketUpdateAction;
	byte marketUpdateAction = (byte)' ';		
	...
	public FixMarketDefinitionUpdateReport() {
		super(FixMessageInfo.MessageTypes.MARKETDEFINITIONUPDATEREPORT);
		...
		hasMarketUpdateAction = FixUtils.TAG_HAS_NO_VALUE;		
	...
	public boolean hasRequiredTags(FixValidationError err) {
		...
		if (!hasMarketReportID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag MarketReportID missing", FixTags.MARKETREPORTID_INT, FixMessageInfo.MessageTypes.MARKETDEFINITIONUPDATEREPORT);
			return false;
		}
		if (!hasMarketID()) { 
			err.setError((int)FixMessageInfo.SessionRejectReason.REQUIRED_TAG_MISSING, "requirde tag MarketID missing", FixTags.MARKETID_INT, FixMessageInfo.MessageTypes.MARKETDEFINITIONUPDATEREPORT);
			return false;
		}
	...
	public int encode(ByteBuffer out) {
		...
		if (hasMarketUpdateAction()) {		
			out.put(FixTags.MARKETUPDATEACTION);		
		
			out.put((byte) '=');		
		
			FixUtils.put(out,marketUpdateAction); 		
	...
	public void setMarketReportID(byte[] src) {		
		if (src == null ) return;
		if (hasMarketReportID()) FixUtils.fillSpace(marketReportID);		
	...
	public void crackMarketUpdateAction() {		
		getMarketUpdateAction();		
	}		
			
	public byte getMarketUpdateAction() { 		
		if ( hasMarketUpdateAction()) {		
			if (hasMarketUpdateAction == FixUtils.TAG_HAS_VALUE) {		
			...
			hasMarketUpdateAction = FixUtils.TAG_HAS_VALUE;		
			buf.position(0);		
			return marketUpdateAction;		
		} else {		
			return (byte)'0'; 		
	...
	public boolean hasMarketUpdateAction() { return hasMarketUpdateAction != FixUtils.TAG_HAS_NO_VALUE; } 		

	public void setMarketUpdateAction(byte[] src) {		
		if (src == null ) return;
		if (hasMarketUpdateAction()) marketUpdateAction = (byte)' ';		
		marketUpdateAction = src[0];		
		hasMarketUpdateAction = FixUtils.TAG_HAS_VALUE;		
	}

	public void setMarketUpdateAction(byte src) {		
		marketUpdateAction = src;
		hasMarketUpdateAction = FixUtils.TAG_HAS_VALUE;		
	}

	public void setMarketUpdateAction(String str) {		
		if (str == null ) return;
		if (hasMarketUpdateAction()) marketUpdateAction = (byte)' ';		
		byte[] src = str.getBytes(); 		
		marketUpdateAction = src[0];		
	...
	public void setMarketSegmentDesc(byte[] src) {		
		if (src == null ) return;
		if (hasMarketSegmentDesc()) FixUtils.fillSpace(marketSegmentDesc);		
	...
	public void setEncodedMktSegmDesc(byte[] src) {		
		if (src == null ) return;
		if (hasEncodedMktSegmDesc()) FixUtils.fillSpace(encodedMktSegmDesc);		
	...
	**/
	...
	public String toString() {
		...
		if (hasMarketUpdateAction()) s += "MarketUpdateAction(1395)= " + getMarketUpdateAction() + "\n" ; 
	...
	public boolean equals(Object o) {
		...
		if ((hasMarketUpdateAction() && !msg.hasMarketUpdateAction()) || (!hasMarketUpdateAction() && msg.hasMarketUpdateAction())) return false;
		if (!(!hasMarketUpdateAction() && !msg.hasMarketUpdateAction()) && !(getMarketUpdateAction()==msg.getMarketUpdateAction())) return false;
	...
	@Override
	public FixMarketDefinitionUpdateReport clone () {
		FixMarketDefinitionUpdateReport out = new FixMarketDefinitionUpdateReport();
	...
	public FixMarketDefinitionUpdateReport clone ( FixMarketDefinitionUpdateReport out ) {
		if ( hasMarketReportID())
			out.setMarketReportID(getMarketReportID());
		if ( hasMarketReqID())
			out.setMarketReqID(getMarketReqID());
		if ( hasMarketUpdateAction())
			out.setMarketUpdateAction(getMarketUpdateAction());
		if ( hasMarketID())
