...
public class FixNestedParties2 extends FixGroup {
        			...
        			if ( tag == FixTags.NONESTED2PARTYSUBIDS_INT ) {
        				...
        				while ( count < noInGroupNumber ) {
        					if ( !nstdPtys2SubGrp[count].isKeyTag(repeatingGroupTag) ) {
        						err.setError((int)FixMessageInfo.SessionRejectReason.INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP, "Incorrect NumInGroup count for repeating group", FixTags.NONESTED2PARTYSUBIDS_INT);
        						return repeatingGroupTag;
        					}
        					count++;
	...
	public byte[] getNested2PartyID() { 		
		if ( hasNested2PartyID()) {		
			...
			FixUtils.getTagStringValue(buf, nested2PartyID, 0, nested2PartyID.length, err);
	...
	public void setNested2PartyID(byte[] src) {		
		if (src == null ) return;
		if (hasNested2PartyID()) FixUtils.fillNul(nested2PartyID);		
	...
	public byte getNested2PartyIDSource() { 		
		if ( hasNested2PartyIDSource()) {		
			...
			nested2PartyIDSource = FixUtils.getTagCharValue(buf, err);
	...
	public long getNested2PartyRole() { 		
		if ( hasNested2PartyRole()) {		
			...
			nested2PartyRole = FixUtils.getTagIntValue(buf, err);
